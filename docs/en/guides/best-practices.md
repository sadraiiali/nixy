...
:::{note}
[Channels](/pages/nix-manual/command-ref/nix-channel) are a mechanism for referencing remote Nix expressions and retrieving their latest version.
:::
...
The [attribute set update operator](/pages/nix-manual/language/operators#update) merges two attribute sets.
...
:::{tip}
Use [`builtins.path`](/pages/nix-manual/language/builtins-prefix#builtins-path) with the `name` attribute set to something fixed.
