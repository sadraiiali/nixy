...
Nix can be configured to use a binary cache with the [`substituters`](/pages/nix-manual/command-ref/conf-file-prefix#conf-substituters) and [`trusted-public-keys`](/pages/nix-manual/command-ref/conf-file-prefix#conf-trusted-public-keys) settings, either exclusively or in addition to cache.nixos.org.
...
For example, given a binary cache at `https://example.org` with public key `My56...Q==%`, and some derivation in `default.nix`, make Nix exclusively use that cache once by passing [settings as command line flags](/pages/nix-manual/command-ref/conf-file-prefix#command-line-flags):

```shell-session
$ nix-build --substituters https://example.org --trusted-public-keys example.org:My56...Q==%
```

To permanently configure trying the custom cache before the public cache, add it as `extra-substiters` with lower `priority` value to the [Nix configuration file](/pages/nix-manual/command-ref/conf-file-prefix#configuration-file):
