...
`nix-build` will call the top-level function with the empty attribute set `{}`, or with the attributes passed via [`--arg`](/pages/nix-manual/command-ref/nix-build#opt-arg) or [`--argstr`](/pages/nix-manual/command-ref/nix-build#opt-argstr).
...
[v3 command line]: /pages/nix-manual/command-ref/experimental-commands
