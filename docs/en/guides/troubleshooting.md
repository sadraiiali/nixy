...
Pass [`--option substitute false`](/pages/nix-manual/command-ref/conf-file-prefix#conf-substitute) to Nix commands.
...
Pass the [`--narinfo-cache-negative-ttl`](/pages/nix-manual/command-ref/conf-file-prefix#conf-narinfo-cache-negative-ttl) option to set the cache timeout in seconds.
...
Which will print the errors in the [database](/pages/nix-manual/glossary#gloss-nix-database).
...
It means that using a new version of Nix upgraded the SQLite schema of the [database](/pages/nix-manual/glossary#gloss-nix-database), and then you tried to use an older version of Nix.
...
This may mean you are trying to import too large a file or directory into the [Nix store](/pages/nix-manual/glossary#gloss-store), or your machine is running out of resources, such as disk space or memory.

Try to reduce the size of the directory to import, or run [garbage collection](/pages/nix-manual/command-ref/nix-collect-garbage).

## macOS update breaks Nix installation

This is a [known issue](https://github.com/NixOS/nix/issues/3616).
The [Nix installer](/pages/nix-manual/installation/installing-binary) modifies `/etc/zshrc`.
