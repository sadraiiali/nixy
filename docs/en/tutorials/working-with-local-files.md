...
To build a local project in a Nix derivation, source files must be accessible to its [`builder` executable](/pages/nix-manual/language/derivations#attr-builder).
By default, the `builder` runs in an [isolated environment](/pages/nix-manual/command-ref/conf-file-prefix#conf-sandbox) that only allows reading from the Nix store.
...
- The [`builtins.path`](/pages/nix-manual/language/builtins-prefix#builtins-path) function
...
You can explore and learn about the library with [`nix repl`](/pages/nix-manual/command-ref/experimental-commands):
...
All functions that expect a file set for an argument can also accept a [path](/pages/nix-manual/language/values#type-path).
...
:::{warning}
When using the [`flakes` and `nix-command` experimental features](/pages/nix-manual/command-ref/experimental-commands),
...
In addition to the included file, this also prints its [file type](/pages/nix-manual/language/builtins-prefix#builtins-readFileType).
