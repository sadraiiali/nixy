...
When building the attribute `hello`, by accessing it with the [`-A` / `--attr` option](/pages/nix-manual/command-ref/nix-build#opt-attr), the result will be the same as before:
