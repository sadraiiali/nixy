...
A binary cache stores pre-built [Nix store objects](/pages/nix-manual/store/store-object) and provides them to other machines over the network.
...
If your binary cache is already a [remote build machine](/pages/nix-manual/advanced-topics/distributed-builds), it will serve all store objects in its Nix store.
...
- The [SSH Store](/pages/nix-manual/store/types), [Experimental SSH Store](/pages/nix-manual/store/types), and the [S3 Binary Cache Store](/pages/nix-manual/store/types) can also be used to serve a cache.
...
- [Nix Manual on HTTP Binary Cache Store](/pages/nix-manual/store/types)
