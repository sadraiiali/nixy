...
- The positional argument to [`nix-build`](/pages/nix-manual/command-ref/nix-build) is a path to the derivation to be built.
...
- The [`-A` option](/pages/nix-manual/command-ref/opt-common#opt-attr) specifies the attribute to pick from the provided Nix expression `<nixpkgs/nixos>`.
...
- The [`-I` option](/pages/nix-manual/command-ref/opt-common#opt-I) prepends entries to the search path.
...
- [Nix manual: `nix-build`](/pages/nix-manual/command-ref/nix-build).
- [Nix manual: common command-line options](/pages/nix-manual/command-ref/opt-common).
