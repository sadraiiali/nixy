...
:::{note}
- [Attribute set syntax](/pages/nix-manual/language/syntax#attrs-literal): attribute names usually do not need quotes
- [List syntax](/pages/nix-manual/language/syntax#list-literal): list elements are separated by white space
:::
...
The `+` sign in the above expression is the [string concatenation operator](/pages/nix-manual/language/operators#string-concatenation), which takes two strings and produces a new string.
...
For details check the [documentation on string literals in the Nix language](/pages/nix-manual/language/syntax#string-literal).
:::
...
The value of a [lookup path](/pages/nix-manual/language/constructs/lookup-path) is a file system path that depends on the value of [`builtins.nixPath`](/pages/nix-manual/language/builtins-prefix#builtins-nixPath).
...
:::{note}
Nix supports other types of impure expressions, such as [lookup paths](search-path) or the constant [`builtins.currentSystem`](/pages/nix-manual/language/builtins-prefix#builtins-currentSystem).
