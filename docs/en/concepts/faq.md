...
:::{tip}
Consult the [`nix-channel`](/pages/nix-manual/command-ref/nix-channel) entry in the Nix Reference Manual for more information on channels, and the [Nixpkgs contributing guide](https://github.com/NixOS/nixpkgs/blob/master/CONTRIBUTING.md#branch-conventions) on the Nixpkgs branching strategy.
:::
...
- The filesystem used for building (see also [`TMPDIR`](/pages/nix-manual/command-ref/env-common#env-TMPDIR)).
