...
آن‌ها ساخت برنامه‌ها را با نسخه‌ی یکسان آسان می‌کنند.
...
این مورد را می‌توان از طریق [command-line] یا گزینه‌ی [`nix.registry`] در NixOS گسترش داد.
...
فلیک‌ها یک فرمت توسعه‌ی تجربی با مسائل حل‌نشده هستند.
عملکرد آن‌ها را به‌طور کلی می‌توان بدون استفاده از خود آن‌ها نیز به دست آورد.

اگر نیاز به اجرای نرم‌افزار موجودی دارید که از قبل از فلیک‌ها استفاده می‌کرده‌است، یا می‌خواهید در توسعه‌ی آن‌ها مشارکت کنید، با خیال راحت از آن‌ها استفاده کنید.
اگر می‌خواهید خودتان کد Nix بنویسید، راهنمای ما درباره‌ی [dependency management] را نیز در نظر بگیرید.[^flake-inputs]
...
- فلیک‌ها هیچ [parameters] ندارند.
  این یعنی `flake.nix` و کاربر نهایی آن باید درباره‌ی [`system`] استفاده‌شده صریح باشند.
...
برای مثال، برای اجرای بسته `hello` از {term}`Nixpkgs`:
...
برای اجرای بسته `hello` از شاخه `nixpkgs-unstable` در {term}`Nixpkgs`:
...
- ساخت‌ها کل پوشه فلیک را در انبار نیکس کپی می‌کنند.
  این کار آن‌ها را کش می‌کند، اما برای مخزن‌های حجیمی مانند {term}`Nixpkgs` می‌تواند [کندتر] باشد.
...
- در {term}`NixOS`، می‌توانید با تنظیم [`nixpkgs.flake.source = pkgs.path;`] در پیکربندی NixOS خود، بخش `nixpkgs` در دستورات v3 را به یک مجموعه بسته `pkgs` تبدیل کنید.
...
- وابستگی‌ها به‌صورت اشتیاق‌آمیز دریافت می‌شوند و وابستگی‌هایی را بارگیری می‌کنند که ممکن است از آن‌ها استفاده نکنید.
...
- فرآیند
  - در حالی که هنوز نگرانی‌های حل‌نشده‌ای درباره‌ی طراحی وجود داشت،
...
- جامعه کاربری
  - این طراحی توسط تمام بخش‌های جامعه کاربری پذیرفته نشده بود، به عنوان مثال {term}`Nixpkgs` از آن در ابزارهای داخلی خود استفاده نکرد.
    ...
    در حالی که انشعاب Nix مبتنی بر جامعه‌ی کاربری یعنی Lix [مجموعه ویژگی‌ها را یکپارچه کرد] تا به عنوان یک «نسخه ۱» عملی تبدیل شود.
    چنین انشعاب‌هایی به طور بالقوه می‌توانند وعده‌ی یک رابط یکپارچه را که در وهله اول آزمایش فلیک‌ها را به جلو راند، نقض کنند.
