...
- فایل‌های سادهٔ نیکس را می‌توان با [دستورات v2] (مانند [`nix-build`]، [`nix-shell`]) یا با [`--file` flag] یا `-f` در دستورات v3 استفاده کرد.
...
در جامعهٔ کاربری، این سبک کدنویسی الگوی درندریت (dendritic pattern) نامیده می‌شود.
...
- فرآیند
  - در حالی که هنوز نگرانی‌های حل‌نشده‌ای دربارهٔ طراحی وجود داشت،
