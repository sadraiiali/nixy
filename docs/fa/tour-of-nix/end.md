...
* [ویکی NixOS](https://nixos.wiki/)
  ...
  نمونه‌های کاربردی عملی، مانند [برگهٔ تقلب (Cheatsheet)](https://nixos.wiki/wiki/Cheatsheet).
...
* `ctrl+s` - سوالات را در فایل `questions.json` داخل پوشهٔ `downloads` شما ذخیره می‌کند

اگر می‌خواهید سوالات جدیدی اضافه کنید، از کنسول جاوااسکریپت استفاده کنید.

**هشدار:** میانبر `ctrl+shift+i` در مرورگر کروم کار نمی‌کند، بنابراین برای کلیک راست (RMB) و گزینهٔ `inspect element` از ماوس استفاده کنید. از طریق همان کنسول جاوااسکریپت می‌توانید شیء `questions` را که در بردارنده تمام سوالات است، گسترش دهید.
