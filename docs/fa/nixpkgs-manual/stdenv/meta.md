# صفات متاداده {#chap-meta}

بسته‌های Nix می‌توانند *صفات متاداده* را اظهار کنند که حاوی اطلاعاتی درباره‌ی یک بسته مانند توضیحات، صفحه خانگی،
...
## متا-صفت‌های استاندارد {#sec-standard-meta-attributes}
...
- معیوب است در صورتی که تمامی وابستگی‌های مجموعه‌ی مشخصی از آن معیوب باشند
```nix
  {
    meta.broken = lib.all (
      map (p: p.meta.broken) [
...
## شناسه‌های نرم‌افزار {#sec-meta-identifiers}
...
یک صفت (attribute) فقط‌خواندنی شامل فهرستی از حدس‌ها درباره‌ی این‌که CPE برای این بسته چگونه می‌تواند باشد. این صفت شامل تمامی گونه‌های مدیریت نسخه اشاره‌شده در بالا است. هر عنصر یک attrset با صفات `cpeParts` و `
