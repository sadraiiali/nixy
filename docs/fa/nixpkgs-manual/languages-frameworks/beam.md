...
سازنده (Builder) `beamPackages.buildRebar3` می‌تواند برای ساخت یک derivation استفاده شود که نحوه‌ی ساخت پروژه‌ی Rebar3 را متوجه می‌شود.
...
هنگام نوشتن یک پروژه‌ی Elixir با هدف‌گیری `mixRelease`، می‌توانید استفاده از [deps_nix](https://github.com/code-supply/deps_nix) همراه با `mixNixDeps` را نیز مد نظر قرار دهید. ابزار `deps_nix` از وابستگی‌های git پشتیبانی می‌کند، اما قصد دارد مستقیماً به `mix.exs` پروژه اضافه شود.
...
- باید نسخه را به صورت مصنوعی در mix.exs ثابت کنید و mix.lock را با نسخه‌ی ثابت دوباره تولید کنید (در مخزن بالادست). این کار به شما امکان می‌دهد `mix2nix > mix_deps.nix` را اجرا کنید.
...
اولین ساخت خطایی درباره‌ی مقدار هش خواهد داد، پس از آن می‌توانید آن را با مقدار پیشنهادی جایگزین کنید.
