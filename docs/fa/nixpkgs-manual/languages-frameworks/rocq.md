...
توجه داشته باشید که "The Rocq Prover" (به اختصار Rocq) نام جدید دستیار اثبات است که قبلاً با نام Coq شناخته می‌شد. درایویشن‌های `coq` و `coqPackages` در حال حاضر هم برای نسخه‌های قدیمی‌تر Coq و هم برای برخی از نسخه‌های Rocq در طول دوره‌ی گذار تغییر نام باقی می‌مانند. در حالت
...
* `version` (اختیاری، با مقدار پیش‌فرض `null`)، نسخه‌ای است که باید دریافت و ساخته شود؛ این صفت بسته به نوع و الگوی آن به چند روش تفسیر می‌شود:
  * اگر یک رشته‌ی نسخه‌ی منتشرشده‌ی شناخته‌شده باشد (مثلاً از صفت `release` در زیر)، انتشار مربوطه انتخاب می‌شود و صفت `version` در derivation حاصل روی این رشته‌ی انتشار تنظیم می‌شود،
  * اگر یک پیشوند majorMinor به صورت `"x.y"` از یک نسخه‌ی منتشرشده‌ی شناخته‌شده باشد (طبق تعریف بالا)، آخرین نسخه‌ی منتشرشده‌ی شناخته‌شده با الگوی `"x.y.z"` انتخاب می‌شود (بر اساس ترتیب تعیین‌شده توسط `versionAtLeast`)،
  * اگر یک مسیر یا رشته‌ای نماینده‌ی یک مسیر مطلق باشد (یعنی با `"/"` شروع
