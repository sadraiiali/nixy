...
`pubspecLock` همان فایل تجزیه‌شدهٔ pubspec.lock است. pub2nix از آن برای دانلود بسته‌های مورد نیاز استفاده می‌کند.
...
::: {.warning}
هنگام استفاده از `autoPubspecLock` با یک پوشهٔ کد منبع محلی، مطمئن شوید که از عملگر الحاق (مانند `autoPubspecLock = src + "/pubspec.lock";`) استفاده می‌کنید و نه درون‌گذاری رشته.

درون‌گذاری رشته تمام پوشهٔ کد منبع شما را در انبار نیکس (Nix store) کپی کرده و از مسیر انبار آن استفاده می‌کند؛ بدین معنا که تغییرات غیرمرتبط در درخت کد منبع شما باعث بازسازی درایویشن تولیدشدهٔ `pubspec.lock` خواهد شد!
:::
