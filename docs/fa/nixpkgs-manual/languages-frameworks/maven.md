...
## ساخت یک بسته با استفاده از `maven.buildMavenPackage` {#maven-buildmavenpackage}
...
### ساخت آفلاین {#maven-offline-build}

به طور پیش‌فرض، `buildMavenPackage` موارد زیر را انجام می‌دهد:

1. اجرای `mvn package -Dmaven.repo.local=$out/.m2 ${mvnParameters}` در `fetchedMavenDeps`
```nix
...
### پلاگین‌های پایدار Maven {#stable-maven-plugins}
...
## استفاده دستی از `mvn2nix` {#maven-mvn2nix}
::: {.warning}
این روش دیگر
...
### ساخت یک فایل JAR {#building-a-jar}
...
### فایل JAR قابل اجرا {#runnable-jar}
...
#### فایل MANIFEST از طریق افزونه Maven {#manifest-file-via-maven-plugin}
