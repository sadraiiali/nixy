...
## برطرف کردن بن‌بست / یافتن نمونه‌های کد {#javascript-finding-examples}
...
### استفاده مستقیم از node_modules {#javascript-using-node_modules}
...
#### آرگومان‌ها {#javascript-buildNpmPackage-arguments}
...
:::{.example}
##### نمونه استفاده از `pkgs.importNpmLock` همراه با `fetcherOpts` {#javascript-buildNpmPackage-example-fetcherOpts}
...
#### فضاهای کاری PNPM {#javascript-pnpm-workspaces}
...
#### دستورات و تنظیمات اضافی PNPM {#javascript-pnpm-extraCommands}
...
##### پچ کردن فایل‌های `package.json` یا `yarn.lock` بالادستی {#javascript-yarnBerry-patching}
...
## خارج از Nixpkgs {#javascript-outside-nixpkgs}
