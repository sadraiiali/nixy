...
## نحوه استفاده از Agda {#how-to-use-agda}
...
اطلاعات بیشتر را می‌توانید در [مستندات رسمی Agda درباره‌ی مدیریت کتابخانه](https://agda.readthedocs.io/en/v2.6.1/tools/package-system.html) بیابید.

## کامپایل کردن Agda {#compiling-agda}
...
## نوشتن بسته‌های Agda {#writing-agda-packages}
...
### ساخت بسته‌های Agda {#building-agda-packages}
...
### نصب بسته‌های Agda {#installing-agda-packages}
...
به طور پیش‌فرض، سورس‌های Agda فایل‌هایی هستند که به `.agda` ختم می‌شوند، یا فایل‌های literate Agda که به `.lagda`، `.lagda.tex`، `.lagda.org`، `.lagda.md` یا `.lagda.rst` ختم می‌شوند. فهرست پسوندهای سورس شناخته‌شده‌ی Agda را می‌توان با تنظیم متغیر پیکربندی `extraExtensions` گسترش داد.

## نگهداری مجموعه بسته‌های Agda در Nixpkgs {#maintaining-the-agda-package-set-on-nixpkgs}
...
### افزودن بسته‌های Agda به Nixpkgs {#adding-agda-packages-to-nixpkgs}
...
### نگه‌داری بسته‌های Agda {#agda-maintaining-packages}
...
ارتقای نسخه‌ی آن وابستگی معکوس باید یک کامیت بعدی روی PR شما باشد.

در موارد نادری که انتظار نمی‌رود انتشار جدیدی در زمانی پذیرفتنی صورت گیرد،
بسته‌ی شکست‌خورده را با تنظیم `meta.broken = true;` به عنوان خراب مشخص کنید.
