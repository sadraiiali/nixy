...
```
`myVimPackage` یک نام دلخواه برای بسته‌ی تولیدشده است. می‌توانید هر نامی را که دوست دارید انتخاب کنید.
...
## ویژگی‌های خاص برخی پلاگین‌ها {#neovim-plugin-specificities}

### پیکربندی اختیاری پلاگین {#neovim-plugin-required-snippet}
...
### بازنشانی‌های مجوز پلاگین {#neovim-plugin-license-overrides}

پلاگین‌های تولیدشده‌ی Vim و Neovim در صورت امکان، `meta.license` خود را از متادیتای مجوز GitHub دریافت می‌کنند.
...
### راه‌اندازی Treesitter با استفاده از پارسرها و کوئری‌های WASM {#neovim-plugin-treesitter-wasm}
...
### گرامرهای Treesitter به عنوان وابستگی‌های پلاگین {#neovim-plugin-treesitter-grammar-dependencies}
