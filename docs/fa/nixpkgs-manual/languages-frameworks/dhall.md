...
… و اکنون Prelude حاوی نتیجه‌ی کاملاً رمزگشایی‌شده‌ی تفسیر Prelude خواهد بود:
...
* `source`: مقدار آن را برابر `true` قرار دهید تا نتیجه‌ی رمزگشایی‌شده به عنوان `source.dhall` در فرآورده‌ی ساخت گنجانده شود، به قیمت مصرف فضای دیسک بیشتر

* `documentationRoot`: اگر می‌خواهید `dhall-docs` مستندات را زیر زیرپوشه‌ی `docs` از فرآورده‌ی ساخت تولید کند، آن را روی پوشه‌ی ریشه‌ی بسته تنظیم کنید
...
* `file`: فایل سطح بالا (به‌طور پیش‌فرض `package.dhall`) که نقطه‌ی ورود به باقی بسته‌است
...
* `directory`: زیرپوشه‌ای از مخزن Git جهت بسته‌بندی (اگر پوشه‌ای غیر از ریشه‌ی مخزن باشد)

* `file`: فایل سطح بالا (به‌طور پیش‌فرض `${directory}/package.dhall`) که نقطه‌ی ورود به باقی بسته‌است
...
`dhall-to-nixpkgs` قابلیت دریافت و ساخت درون‌ریزی‌های راه دور را به عنوان درایویشن‌های با خروجی ثابت با استفاده از بررسی یکپارچگی Dhall آن‌ها دارد. این روش گاهی از بسته‌بندی دستی همه‌ی درون‌ریزی‌های راه دور آسان‌تر است.
...
… زیرا Prelude پیش‌فرض انتخاب‌شده توسط بازبینی `94b2848559b12a8ed1fe433084686b2a81123c99` از Nixpkgs، نسخه 20.1.0 است که بررسی تمامیت یکسانی با نسخه 19.0.0 ندارد. این بدان معناست که نسخه 19.0.0 ذخیره‌شده در حافظه‌ی پنهان نیست و مفسر مجاز نیست به درون‌ریزی URL بازگردد.
