...
با استفاده از `nix-build default.nix` اکنون می‌توانیم پروژه خود را ساخت دهیم، اما همچنین می‌توانیم با استفاده از `nix-shell -A env default.nix` وارد شلی شویم که همه‌ی وابستگی‌های بسته در آن در دسترس هستند. اگر `cabal-install` را به صورت سراسری نصب کرده باشید، طبق انتظار در داخل شل کار خواهد کرد.
