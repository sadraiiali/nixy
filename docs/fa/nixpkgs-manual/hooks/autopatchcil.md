...
این یک قلاب راه‌اندازی (setup hook) ویژه است که به بسته‌بندی برنامه‌ها/اسمبلی‌های .NET کمک می‌کند؛ به این صورت که به‌طور خودکار تلاش می‌کند وابستگی‌های کتابخانه‌ای اشتراکیِ مفقودشدهٔ اسمبلی‌های .NET را بر اساس `buildInputs` و `nativeBuildInputs` داده‌شده پیدا کند.
...
دستور `autoPatchcil` به پرچم خط فرمان `--rid` نیاز دارد تا RID (شناسهٔ زمان اجرا) فرضی برای اجرای اسمبلی‌ها را مشخص کند، و همچنین پرچم خط فرمان `--no-recurse` را می‌شناسد که از پیمایش بازگشتی درون زیرپوشه‌ها جلوگیری می‌کند.
