...
## متغیرها {#sec-postgresqlTestHook-variables}
...
متغیرهای صادرشده:
 ...
 - `PGHOST`: مکان پوشه‌ی سوکت دامنه‌ی یونیکس؛ `host` پیش‌فرض در یک رشته‌ی اتصال (connection string).
...
## قلاب‌ها {#sec-postgresqlTestHook-hooks}
...
## TCP و ایزوله‌سازی Nix {#sec-postgresqlTestHook-tcp}
...
::: {.note}
خطای `libpq: failed (could not receive data from server: Connection refused` عموماً نشان‌دهنده‌ی این است که مجموعه تست در حال تلاش برای برقراری اتصال از طریق TCP است.
:::
