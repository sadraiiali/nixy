...
این کار در یک محیط تمیز (با استفاده از `env --ignore-environment`) انجام می‌شود و خروجی‌های `stdout` و `stderr` دستور را برای یافتن رشته `${version}` بررسی می‌کند. خروجی دریافت‌شده را در لاگ ساخت به شما گزارش می‌دهد و اگر نتواند `${version}` را پیدا کند، ساخت با شکست مواجه خواهد شد.

متغیرهایی که این فاز کنترل می‌کند عبارت‌اند از:

- `dontVersionCheck`: افزودن این قلاب به [`preInstallCheckHooks`](#ssec-installCheck-phase) را غیرفعال می‌کند. زمانی مفید است که بخواهید توابع Bash قلاب را بارگذاری کنید، اما آن‌ها را به شیوه‌ی دیگری اجرا نمایید.
- `versionCheckProgram`: مسیر کامل برنامه‌ای که باید رشته `${version}` را چاپ کند. به طور پیش‌فرض به ترتیب از اولین مقدار غیرخالی `$binary` در میان `${NIX_MAIN_PROGRAM}` و `${pname}` استفاده می‌کند تا تقریباً `${placeholder "out"}/bin/$binary` را بسازد. مقدار `${NIX_MAIN_PROGRAM}` از `meta.mainProgram` می‌آید و معمولاً نیازی نیست به طور صریح تنظیم شود. هنگام تنظیم `versionCheckProgram`، استفاده مستقیم از `$out` کار نخواهد کرد، زیرا متغیرهای محیطی از این متغیر توسط قلاب بازگشایی نمی‌شوند. بنابراین استفاده از `placeholder "out"` اجتانب‌ناپذیر است.
