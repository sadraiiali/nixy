...
## پیکربندی دستی هسته (kernel) {#sec-manual-kernel-configuration}

گاهی اوقات ممکن است استفاده از هسته‌های ساخته‌شده با `pkgs.buildLinux` مطلوب نباشد، به‌ویژه اگر لازم باشد اکثر پیکربندی‌های رایج تغییر داده شوند یا غیرفعال شوند تا هسته‌ای مطابق با مورد استفاده‌ی هدف به دست آید.
...
## توسعه‌ی ماژول‌های هسته {#sec-linux-kernel-developing-modules}

هنگام توسعه‌ی ماژول‌های هسته، اغلب بسیار مناسب است که چرخه‌ی ویرایش-کامپایل-اجرا را در سریع‌ترین زمان ممکن انجام دهید.
قطعه‌کد زیر را به عنوان یک مثال ببینید.

:::{.example #ex-edit-compile-run-kernel-modules}

# چرخه‌ی ویرایش-کامپایل-اجرا هنگام توسعه‌ی درایورهای `mellanox`
...
## اطلاعات نگه‌دارنده {#sec-linux-kernel-maintainer-information}

### به‌روزرسانی هسته‌ها {#sec-linux-updates}
