...
برای پلتفرم‌های `linux`، تابع `buildContainer` همچنین فضاهای نام زیر را پیکربندی می‌کند (ببینید: {manpage}`unshare(1)`) تا کنتینر OCI را از فضای نام سراسری ایزوله کند:
...
### مثال‌ها {#ssec-pkgs-ociTools-buildContainer-examples}
...
به‌عنوان مثالی از نحوه‌ی اجرای کنتینر تولیدشده توسط این بسته، از `runc` برای راه‌اندازی کنتینر استفاده خواهیم کرد.
