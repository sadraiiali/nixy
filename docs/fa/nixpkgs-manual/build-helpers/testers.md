...
: یک مجموعه ویژگی که در آن نام صفات عبارت‌های منظم (regular expression) هستند.
  ...
  با افزودن نگاشت‌های مجدد، می‌توانید با ارائهٔ یک جایگزین از سیستم‌فایل، به‌صورت آفلاین درست بودن URLهای مربوط به منابع خارجی را بررسی کنید.
...
علاوه بر این، کاربران می‌توانند یک اسکریپت حاوی بررسی‌های تکمیلی مشخص کنند و از طریق متغیر `failed` به نتیجهٔ اعمال `testers.testBuildFailure` دسترسی داشته باشند.
...
: رشته‌ای شامل بررسی‌های اضافی جهت اجرا.
  در صورت عدم ارائه، مقدار پیش‌فرض آن برابر با `""` است.
  نتیجهٔ `testers.testBuildFailure drv` از طریق متغیر `failed` در دسترس است.
