# دریافت‌کننده‌ها {#chap-pkgs-fetchers}
...
:::{.tip}
کمک‌رسان‌های `pkgs.fetchFrom*` به جای کل تاریخچه‌ی نسخه‌ها، _اسنپ‌شات‌های_ کدهای منبع تحت کنترل نسخه را دریافت می‌کنند که کارآمدتر است.
`pkgs.fetchgit` به طور پیش‌فرض نیز همین رفتار را دارد، اما می‌توان آن را از طریق صفات خاصی که به آن داده می‌شود تغییر داد.
:::

## هشدارها {#chap-pkgs-fetchers-caveats}
...
- هنگام تغییر هر یک از پارامترهای دریافت‌کننده، همیشه هش خروجی را به‌روزرسانی کنید.
  ...
  تابع [`invalidateFetcherByDrvHash`](#tester-invalidateFetcherByDrvHash) به جلوگیری از استفاده‌ی مجدد درایویشن‌های ذخیره‌شده در کش کمک می‌کند.
  :::

## به‌روزرسانی هش‌های کد منبع {#sec-pkgs-fetchers-updating-source-hashes}
...
1. []{#sec-pkgs-fetchers-updating-source-hashes-fakehash-method} روش هش ساختگی: در دستورالعمل بسته خود، هش را روی یکی از موارد زیر قرار دهید
...
## دریافت امن هش‌ها {#sec-pkgs-fetchers-secure-hashes}
...
## استفاده از پروکسی {#sec-pkgs-fetchers-proxy}
...
این تابع به صورت داخلی از {manpage}`curl(1)` استفاده می‌کند و اجازه می‌دهد رفتار آن با مشخص کردن چند صفت (attribute) در آرگومانِ `fetchurl` تغییر کند (مستندات صفت‌های `curlOpts`، `curlOptsList` و `netrcPhase` را ببینید).
...
### ورودی‌ها {#sec-pkgs-fetchers-fetchurl-inputs}
...
: هش خروجی derivation مربوط به `fetchurl` به فرمتی که Nix انتظار دارد.
  برای اطلاعات بیشتر درباره‌ی فرمت آن، [مستندات راهنمای Nix](https://nixos.org/manual/nix/stable/language/advanced-attributes.html#adv-attr-outputHash) را ببینید.
...
: الگوریتم مورد استفاده برای تولید مقدار مشخص‌شده در `outputHash`.
  برای اطلاعات بیشتر درباره‌ی مقادیری که پشتیبانی می‌کند، [مستندات راهنمای Nix](https://nixos.org/manual/nix/stable/language/advanced-attributes.html#adv-attr-outputHashAlgo) را ببینید.
...
: هش SHA-1 خروجی derivation / اشتقاق ساختِ `fetchurl` به قالبی که مورد انتظار Nix است.
  برای اطلاعات بیشتر درباره‌ی قالب آن، [مستندات راهنمای Nix](https://nixos.org/manual/nix/stable/language/advanced-attributes.html#adv-attr-outputHash) را ببینید.
...
: نام نمادین فایل بارگیری‌شده هنگام ذخیره در انبار نیکس (Nix store).
  برای جزئیات درباره‌ی نحوه تعیین نام فایل، [بررسی اجمالی `fetchurl`](#sec-pkgs-fetchers-fetchurl) را ببینید.
...
`recursiveHash` (بولین؛ _اختیاری_) []{#sec-pkgs-fetchers-fetchurl-inputs-recursiveHash}
...
`downloadToTemp` (بولین؛ _اختیاری_) []{#sec-pkgs-fetchers-fetchurl-inputs-downloadToTemp}
: اگر `true` باشد، فایل بارگیری‌شده را به جای مکان مورد انتظار در انبار نیکس (Nix store)، در یک مکان موقت ذخیره می‌کند.
  این حالت هنگام استفاده در کنار صفت (attribute) `postFetch` مفید است، در غیر این صورت `fetchurl` هیچ خروجی بامعنایی تولید نخواهد کرد.
...
: اسکریپتی که برای ایجاد یک فایل {manpage}`netrc(5)` جهت استفاده با {manpage}`curl(1)` اجرا می‌شود.
  ...
  :::{.note}
  در صورت مشخص شدن، `fetchurl` به طور خودکار فراخوانی {manpage}`curl(1)` را برای استفاده از فایل `netrc` تغییر می‌دهد، بنابراین نیازی به اضافه کردن هیچ چیزی به `curlOpts` یا `curlOptsList` ندارید.
  :::
...
: در صورت مشخص شدن، این مقدار هنگام دانلود URL(های) داده‌شده به `fetchurl` به فراخوانی {manpage}`curl(1)` اضافه می‌شود.
...
: در صورت مشخص شدن، هر عنصر از این فهرست هنگام دانلود URL(های) داده‌شده به `fetchurl` به عنوان یک آرگومان به فراخوانی {manpage}`curl(1)` پاس داده می‌شود.
...
: مشخص‌کننده‌ی هرگونه ویژگی‌های اضافی [`passthru`](#chap-passthru) برای derivation بازگردانده‌شده توسط `fetchurl` است.
...
### خروجی‌های Passthru {#ssec-pkgs-fetchers-fetchurl-passthru-outputs}
...
### نمونه‌ها {#ssec-pkgs-fetchers-fetchurl-examples}
...
برای جزئیات بیشتر درباره‌ی نحوه کار با صفت `hash` هنگام تغییر خروجی، به [](#chap-pkgs-fetchers-caveats) مراجعه کنید.
:::
...
### ورودی‌ها {#sec-pkgs-fetchers-fetchzip-inputs}
...
### نمونه‌ها {#sec-pkgs-fetchers-fetchzip-examples}
...
- `patch` و `hash`: نام فایل پچ،
  و هش آن پس از نرمال‌سازی توسط `fetchpatch`؛ `pname`: نام بسته سورس Debian؛ `version`: شماره نسخه آپ‌استریم (upstream)؛ `debianRevision`: [Debian revision number] (در صورت وجود)؛ `area` مربوط به آرشیو Debian: `main` (پیش‌فرض)، `contrib` یا `non-free`.
...
: وقتی خالی نباشد، فقط محتویات زیرپوشه‌ی مخزن (Repository) را در نتیجه کپی می‌کند. به‌طور خودکار `sparseCheckout` و `nonConeMode` را تنظیم می‌کند تا از دریافت بخش‌های اضافی جلوگیری شود. با `leaveDotGit` ناسازگار است.
...
## `fetchFromGitea`، `fetchFromForgejo` و `fetchFromCodeberg` {#fetchfromgitea}
...
برای استفاده از یک نمونه‌ی مشخص GitLab، از `domain` استفاده کنید (پیش‌فرض آن `"gitlab.com"` است).
...
(شناسه‌ی مخزن *بدون* پیشوند `:rad`) است. همچنین یک آرگومان اختیاری `node` را می‌پذیرد که
شامل شناسه‌ی گرهی است که باید ref مشخص‌شده از آن دریافت شود. اگر `node` برابر با `null` (پیش‌فرض)
...
در غیر این صورت، سازنده (Builder) اجرا می‌شود، اما با پیامی که نحوه‌ی ارائه‌ی فایل را به کاربر توضیح می‌دهد با شکست مواجه می‌شود. برای مثال، کد زیر:
