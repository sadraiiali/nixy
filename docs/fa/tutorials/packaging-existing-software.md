...
این کار شما یا دیگران را قادر می‌سازد تا در نتیجه‌ی «بسته‌بندی نرم‌افزار موجود با Nix» ، یک فرآورده‌ی ساخت برای استفاده‌ی عملی تولید کنید.
:::
...
تا آن نقطه، `stdenv.mkDerivation` به طور خودکار اطلاعات `buildPhase` را برای بسته‌ی `icat` تعیین کرده بود.
