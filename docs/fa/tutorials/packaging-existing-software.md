...
این آموزش با `hello` آغاز می‌شود؛ پیاده‌سازی برنامهٔ «hello world» که تنها نیازمند وابستگی‌هایی است که از پیش توسط `stdenv` فراهم شده‌اند.
...
-->
یک _بسته_ مفهومی است که تعریف آزادی دارد و به مجموعه‌ای از فایل‌ها و داده‌های دیگر یا به یک {term}`عبارت نیکس (Nix expression)` که نمایندهٔ چنین مجموعه‌ای پیش از تحقق یافتن آن است، اشاره دارد.
...
این کار شما یا دیگران را قادر می‌سازد تا در نتیجهٔ «بسته‌بندی نرم‌افزار موجود با Nix» ، یک فرآوردهٔ ساخت برای استفادهٔ عملی تولید کنید.
:::
...
GNU Hello پیاده‌سازی برنامهٔ «سلام دنیا» است که کد منبع آن [از سرور FTP پروژهٔ گنو](https://ftp.gnu.org/gnu/hello/) قابل دسترس است.
...
صفت `hash` را روی یک رشتهٔ خالی تنظیم کنید و سپس با استفاده از پیام خطای حاصل، هش صحیح را مشخص کنید:
...
مجموعهٔ `stdenv` از [`nixpkgs`](https://github.com/NixOS/nixpkgs/) در دسترس است که باید با یک عبارت Nix دیگر درون‌ریزی شود تا به عنوان یک آرگومان به این derivation ارسال گردد.

روش توصیه‌شده برای انجام این کار، ایجاد یک فایل `default.nix` در همان پوشهٔ `hello.nix` با محتوای زیر است:
...
در `hello.nix`، رشتهٔ خالی را با هش صحیح جایگزین کنید:
...
مخزن اصلی `icat` روی [GitHub](https://github.com/atextor/icat) میزبانی می‌شود، بنابراین باید [دریافت‌کنندهٔ کد منبع](https://nixos.org/manual/nixpkgs/stable/#chap-pkgs-fetchers) قبلی را جایگزین کنید.
...
این بار، به جای استفاده از رشتهٔ خالی و اجازه دادن به `nix-build` برای اعلام هش درست در قالب یک خطا، می‌توانید از همان ابتدا هش درست را با دستور `nix-prefetch-url` دریافت کنید.
...
اگر [عبارت `imlib2` را در سایت search.nixos.org جستجو کنید](https://search.nixos.org/packages?query=imlib2)، متوجه خواهید شد که `imlib2` ازپیش در Nixpkgs وجود دارد.
...
اما نکتهٔ مهم برای این آموزش، خطای `fatal error: X11/Xlib.h: No such file or directory` است: یک وابستگی دیگر وجود ندارد.
...
برای محدود کردن نتایج، فقط پوشهٔ `pkgs` را جستجو کنید که تمام دستورالعمل‌های بسته‌ها را در خود نگه می‌دارد:
...
### افزودن مجموعهٔ بسته‌ها به عنوان وابستگی‌ها
...
:::{note}
از آنجا که زبان Nix به‌صورت تنبل ارزیابی می‌شود، دسترسی تنها به `xorg.libX11` به این معنا است که محتویات باقی‌ماندهٔ مجموعه ویژگی `xorg` هرگز پردازش نمی‌شوند.
:::
...
فایل `README` در مخزن `icat` تنها به استفاده از `make` برای ساخت ابزار اشاره می‌کند و مرحلهٔ نصب را به عهدهٔ کاربران می‌گذارد.
...
از آنجا که `make` با موفقیت به پایان می‌رسد، فایل اجرایی `icat` در پوشهٔ ساخت موجود است.
شما فقط باید آن را از آنجا به پوشهٔ خروجی کپی کنید.

در Nix، پوشهٔ خروجی در متغیر `$out` ذخیره می‌شود.
...
یک پوشهٔ `bin` درون پوشهٔ `$out` ایجاد کنید و باینری `icat` را به آنجا کپی کنید:
...
دستور `ls` را در پوشهٔ محلی فراخوانی کنید تا یک پیوند نمادین `result` را که به مسیری در انبار Nix اشاره می‌کند، بیابید:
