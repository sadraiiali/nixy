...
مجموعهٔ بسته‌های نیکس (Nixpkgs) ابزارهایی را برای کامپایل متقاطع نرم‌افزار برای انواع سیستم‌های مختلف فراهم می‌کند.
...
مجموعهٔ بسته‌های نیکس (Nixpkgs) از پشتیبانی بسیار آزمایش‌شده‌ای برای کامپایل متقاطع برخوردار است.
...
مجموعهٔ بسته‌های نیکس (Nixpkgs) دارای مجموعه‌ای از هاست پلتفرم‌های ازپیش‌تعریف‌شده برای کامپایل متقاطع به نام `pkgsCross` است.
...
آن‌ها معمولاً با رشتهٔ پیکربندی پلتفرم متناظر مطابقت ندارند.

می‌توانید رشتهٔ پلتفرم را از `pkgsCross.<platform>.stdenv.hostPlatform.config` بازیابی کنید:
...
- در حالی که بسیاری از کامپایلرها در Nixpkgs از کامپایل متقاطع پشتیبانی می‌کنند، همه آن‌ها چنین قابلیتی ندارند.
 ...
 [توضیحات تفصیلی دربارهٔ نحوه پیاده‌سازی کامپایل متقاطع در Nix](https://nixos.org/manual/nixpkgs/stable/#chap-cross) می‌تواند در رفع آن مسائل کمک کند.

- جامعهٔ کاربری Nix یک [اتاق اختصاصی در Matrix](https://matrix.to/#/#cross-compiling:nixos.org) برای دریافت کمک در زمینه کامپایل متقاطع دارد.
