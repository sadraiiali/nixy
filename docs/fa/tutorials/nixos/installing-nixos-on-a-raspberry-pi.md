...
:::{note}
این آموزش برای رسبری پای 4B نوشته شده‌است. استفاده از مدل‌های پشتیبانی‌شدهٔ قبلی مانند 3B یا 3B+ با اعمال برخی تغییرات در این آموزش امکان‌پذیر است.
:::
...
برای این کار روی آخرین ساخت موفق (که با علامت تیک سبز مشخص شده‌است) کلیک کرده و پیوند مربوط به تصویر فرآوردهٔ ساخت را کپی کنید.
:::
...
اگر **نمی‌خواهید** این اتفاق بیفتد، می‌توانید اعتبارنامه‌های خود را در یک کنسول وارد کنید یا از یکی از راهکارهای جامعهٔ کاربری برای رازهای رمزگذاری‌شده استفاده کنید.
:::
...
- هنگامی که یک سیستم‌عامل کاری در اختیار داشتید، سعی کنید آن را با دستور `nixos-rebuild switch --upgrade` ارتقا دهید تا نسخه‌های بستهٔ به‌روزتری را نصب کنید، و اگر چیزی دچار خرابی شد، به پیکربندی قدیمی‌تر بازگردانی (rollback) کنید.
- برای فعال‌سازی شتاب‌دهی سخت‌افزاری جهت تجربهٔ یک میزکار گرافیکی دلپذیر، ماژول [`nixos-hardware`](https://github.com/nixos/nixos-hardware) را به پیکربندی خود اضافه کنید:
```nix
  imports = [
