...
هنگامی که نمونهٔ AWS در حال اجرای یک تصویر NixOS از طریق Terraform است، می‌توانیم به Terraform بیاموزیم که همیشه جدیدترین پیکربندی NixOS را بسازد و آن تغییرات را روی نمونهٔ شما اعمال کند.
...
- ماژول `deploy_nixos` زمانی که معماری‌های کاربر (client) و مقصد متفاوت باشند کار نمی‌کند (مگر اینکه از [بیلدهای توزیع‌شده](/pages/nix-manual/advanced-topics/distributed-builds) استفاده کنید).
