...
- آشنایی با [زبان Nix](reading-nix-language)
...
- یک *ماشین راه دور* (مثال نام هاست: `remotemachine`)
  ...
  برای راه‌اندازی یک سیستم NixOS راه دور، [](provisioning-remote-machines-tutorial) را دنبال کنید.
...
- فهرستی از [قابلیت‌های پشتیبانی‌شده سیستم](/pages/nix-manual/command-ref/conf-file-prefix#conf-system-features)
  ...
  این فهرست خاص باید مشخص شود تا بتوان ساخت کامپایلرها و اجرای [ماشین‌های مجازی تست یکپارچه‌سازی](integration-testing-vms) را به ماشین‌های راه دور محول کرد.
...
:::{tip}
برای کسب اطلاعات بیشتر درباره‌ی گزینه‌های موجود در [`nix.settings`](https://search.nixos.org/options?show=nix.settings)، به [راهنمای مرجع Nix](/pages/nix-manual/command-ref/conf-file-prefix) مراجعه کنید.
:::
...
:::{tip}
برای جزئیات بیشتر به [مستندات گزینه NixOS درباره‌ی `nix.buildMachines`](https://search.nixos.org/options?query=nix.buildMachines) مراجعه کنید.
:::
