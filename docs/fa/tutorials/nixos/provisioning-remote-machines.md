...
# آموزش پیکربندی و آماده‌سازی ماشین‌های راه دور از طریق SSH
...
*ماشین محلی* فقط به یک [نصب Nix](install-nix) فعال نیاز دارد.
...
[تعیین وابستگی‌ها](dependency-management-npins) روی `nixpkgs`، `disko` و `nixos-anywhere`：
...
رشتهٔ `<your SSH key here>` را با کلید عمومی SSH که می‌خواهید برای ورودهای آینده به عنوان کاربر `root` استفاده کنید، جایگزین کنید.
:::
