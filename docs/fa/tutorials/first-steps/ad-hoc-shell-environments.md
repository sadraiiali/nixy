...
هنگامی که {ref}`نیکس را نصب کردید <install-nix>`، می‌توانید از آن برای ایجاد *محیط‌های شل* جدید با برنامه‌هایی که می‌خواهید استفاده کنید، بهره ببرید.
...
### بررسی نسخه بسته (check-package-version)

بررسی کنید که نسخهٔ مشخصی از این برنامه‌ها را که توسط نیکس ارائه شده‌اند در اختیار دارید، حتی اگر هر یک از آن‌ها را از قبل روی دستگاه خود نصب کرده بودید.
...
- {ref}`reproducible-scripts` استفاده از Nix برای اسکریپت‌های تکرارپذیر
- {ref}`reading-nix-language` یادگیری نحوه خواندن زبان Nix که برای اعلام بسته‌ها و پیکربندی‌ها استفاده می‌شود
- {ref}`declarative-reproducible-envs` ایجاد محیط‌های شل تکرارپذیر با یک فایل پیکربندی اعلانی (declarative)
- {ref}`pinning-nixpkgs` یادگیری روش‌های مختلف مشخص کردن نسخه‌های دقیق منابع بسته
