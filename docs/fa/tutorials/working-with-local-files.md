...
شما می‌توانید با استفاده از [`nix repl`](/pages/nix-manual/command-ref/experimental-commands) کتابخانه را بررسی کرده و دربارهٔ آن بیاموزید:
...
به‌طور ویژه، رویکرد استفاده از `difference ./.` به‌طور صریح فایل‌هایی را که باید _مستثنی شوند_ انتخاب می‌کند، به این معنا که فایل‌های جدید اضافه شده به پوشهٔ منبع به‌طور پیش‌فرض شامل می‌شوند.
بسته به پروژهٔ شما، این روش ممکن است نسبت به روش جایگزین در بخش بعدی مناسب‌تر باشد.
...
این یعنی فایل‌های جدید اضافه شده به پوشهٔ فعلی به‌طور پیش‌فرض نادیده گرفته خواهند شد.
...
این همان خروجی روش دیگر را تولید خواهد کرد و بنابراین از یک نتیجهٔ ساخت قبلی استفاده مجدد می‌کند:
...
ما نمونه‌هایی را از نحوهٔ استفاده از تمام توابع بنیادی مجموعه‌فایل‌ها نشان داده‌ایم.
برای موارد استفادهٔ پیچیده‌تر، آن‌ها را می‌توان در صورت نیاز با یکدیگر ترکیب کرد.

برای مشاهدهٔ فهرست کامل و جزئیات بیشتر، به [مستندات مرجع `lib.fileset`](https://nixos.org/manual/nixpkgs/stable/#sec-functions-library-fileset) مراجعه کنید.
