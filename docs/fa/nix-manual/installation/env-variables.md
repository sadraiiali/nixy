...
برای استفاده از Nix، باید برخی از متغیرهای محیطی تنظیم شوند. به‌طور خاص، متغیر `PATH` باید شامل پوشه‌های `prefix/bin` و `~/.nix-profile/bin` باشد. پوشه‌ی اول حاوی خود ابزارهای Nix است، در حالی که `~/.nix-profile` یک پیوند نمادین (symlink) به *محیط کاربر* فعلی (یک بسته‌ی خودکار تولیدشده متشکل از پیوندهای نمادین به بسته‌های نصب‌شده) است. ساده‌ترین راه برای تنظیم متغیرهای محیطی مورد نیاز، گنجاندن فایل `prefix/etc/profile.d/nix.sh` در فایل `~/.profile` (یا موارد مشابه) است، مانند این:
