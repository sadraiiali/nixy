...
به [مقالهٔ مقادیر ثابت رشته‌ای (String literals)](string-literals.md) مراجعه کنید.
...
فهرستی متشکل از چهار عنصر را تعریف می‌کند که آخرین مورد، نتیجهٔ فراخوانی تابع `f` است. توجه داشته باشید که فراخوانی‌های تابع باید درون پرانتز قرار گیرند. اگر آن‌ها حذف می‌شدند، مثلاً،
...
نام صفت یک رشتهٔ بدون کانتکست است و با یک [نام] (یک [شناسه](./identifiers.md#identifiers) یا [رشتهٔ تحت‌الفظی](string-literals.md)) نشان داده می‌شود.
