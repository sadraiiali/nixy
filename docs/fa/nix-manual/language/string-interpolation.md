...
توجه داشته باشید که عبارت‌ها و رشته‌های Nix می‌توانند به طور دلخواه تو در تو باشند؛ در این حالت، رشتهٔ بیرونی شامل عبارت‌های درون‌گذاری‌شدهٔ مختلفی است که خودشان حاوی رشته‌هایی هستند (مانند `"-thread"` به عنوان مثال)، و برخی از آن‌ها به نوبه خود حاوی عبارت‌های درون‌گذاری‌شده هستند (مانند `${mesa}`).

برای نوشتن یک `${` به صورت صریح در یک رشتهٔ معمولی، آن را با یک اسلش به عقب (`\`) فرار (escape) دهید.
