...
یک *نام* (name) می‌تواند به صورت یک [شناسه](#identifiers) یا یک [رشتهٔ متنی صریح](./string-literals.md) نوشته شود.
...
>
> مشکلات دیرینه‌ای در نحوهٔ تجزیه (parsing) کلمهٔ `or` به عنوان یک نام وجود دارد که بدون ایجاد تغییر ناسازگار (breaking change) در زبان، قابل حل نیستند.
