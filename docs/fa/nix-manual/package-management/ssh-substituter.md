...
شما می‌توانید به Nix بگویید که به طور خودکار باینری‌های مورد نیاز را از یک انبار Nix راه دور از طریق SSH دریافت کند. برای مثال، دستور زیر Firefox را نصب می‌کند و اگر مسیرهای انبار موجود در بسته‌ی بسته‌ی (closure) مربوط به Firefox روی سرور `avalon` موجود باشند، آن‌ها را به صورت خودکار دریافت می‌کند:
