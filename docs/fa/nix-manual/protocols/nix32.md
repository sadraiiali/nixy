...
کدگذاری Nix32 گونه‌ای از کدگذاری [Base32](https://en.wikipedia.org/wiki/Base32) در نیکس است که برای [خلاصه‌های مسیر انبار](@docroot@/protocols/store-path.md)، خروجی هش از طریق [`nix hash`](@docroot@/command-ref/new-cli/nix3-hash.md) و صفت درایویشن [`outputHash`](@docroot@/language/advanced-attributes.md#adv-attr-outputHash) استفاده می‌شود.
