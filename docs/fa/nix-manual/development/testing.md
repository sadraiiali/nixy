...
داده‌های مربوط به تست‌های واحد در یک زیرپوشه `data` متعلق به پوشه‌ی هر فایل اجرایی تست واحد ذخیره می‌شوند.
برای نمونه، کد `libnixstore` در `src/libstore` قرار دارد و داده‌های تست آن در `src/libstore-tests/data` هستند.
مسیر پوشه‌ی `src/${library_name_without-nix}-test/data` از طریق متغیر محیطی `_NIX_TEST_UNIT_DATA` به فایل اجرایی تست واحد ارسال می‌شود.
...
- کار (job) `tests` نصب‌کننده‌ها را برای پلتفرم‌های زیر تولید کرده و آن‌ها را به‌عنوان یک فرآورده‌ی ساخت (artifact) بارگذاری می‌کند:
  - `x86_64-linux`
  - `aarch64-darwin`
...
پس از اتمام اجرای ادغام مداوم، می‌توانید خروجی را بررسی کنید تا فرآورده‌ی ساخت نصب‌کننده را استخراج کنید:
1. روی نمای جزئیات اجرای ادغام مداوم کلیک کنید.
2. به سمت پایین به بخش `Artifacts` اسکرول کنید.
3. فرآورده‌ی ساخت نصب‌کننده‌ی مربوطه را بارگیری کنید (`installer-darwin` برای `aarch64-darwin` و `installer-linux` برای `x86_64-linux`).
4. فرآورده‌ی ساخت `.zip` بارگیری‌شده را از حالت فشرده خارج کنید.
5. برای تولید یک دستور نصب، مسیر فرآورده‌ی ساخت استخراج‌شده را در این الگو قرار دهید:
```console
