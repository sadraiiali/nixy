...
دستورات باید **از گفتگوی اسم-فهرست (noun-verb) پیروی کنند**. اگرچه قالب‌بندی اسم-فهرست از منظر گفتاری معکوس به نظر می‌رسد (یعنی `nix store copy` در برابر `nix copy store`)، اما به ما اجازه می‌دهد دستورات را همان‌گونه که کاربران دربارهٔ انجام یک عمل فکر می‌کنند (ابتدا گروه، سپس دستور) سازمان‌دهی کنیم.
...
- دربارهٔ `ARGUMENTS` و `OPTIONS` در [بخش *ورودی*](#input) بحث شده‌است.
...
- **دستورات اصلی**
  ...
  دستوراتی که برای موارد استفادهٔ اصلی ما به کار می‌روند و بیشترین احتمال استفاده توسط کاربران جدید را دارند. ما انتظار توجه به جزئیات را داریم، از جمله:
...
از آنجا که هیچ روش استانداردی برای نحوهٔ جستجوی کاربر به دنبال راهنما وجود ندارد، ما به روش‌هایی تکیه می‌کنیم که توسط ابزارهای رایج برای ارائهٔ راهنما استفاده می‌شوند. به عنوان راهنمایی برای این موضوع، ما `git` را در نظر گرفتیم و هر زمان که شک داشتیم، آن را به عنوان جهت‌گیری ترجیحی نگاه کردیم.
...
- در انتهای خلاصه یا من‌پِیج باید یک URL وجود داشته باشد که به نسخهٔ آنلاین مستندات دقیق‌تر اشاره کند.
...
نشان دادن اینکه گام‌های بعدی احتمالی چه مواردی هستند و روند توسعهٔ معمول با نیکس چگونه است، می‌تواند برای تازه‌واردان بسیار ارزشمند باشد. برای مثال:
...
`ARGUMENTS` نشان‌دهندهٔ یک ورودی ضروری برای یک تابع است. هنگام انتخاب استفاده از `ARGUMENTS` به جای `OPTIONS` لطفاً از معایب همراه آن آگاه باشید:
...
آنچه ما تشویق به انجام آن می‌کنیم، **ساخت نمونه‌های اولیه (prototypes)**، انجام مقداری **تست کاربری (user testing)** و جمع‌آوری **بازخورد (feedback)** است. سپس تکرار چندبارهٔ این چرخه.
...
**کار باقی‌مانده (TODO)**: پس از اینکه پیاده‌سازی مسیر اصلی (*happy path*) را آماده کردیم، دربارهٔ نحوه نمایش خطاها فکر خواهیم کرد.
