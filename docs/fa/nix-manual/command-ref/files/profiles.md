...
نسخه‌بندی پروفایل‌ها به شکل زیر است. هنگام استفاده از یک پروفایل به نام *path*، متغیر *path* یک پیوند نمادین به *path*`-`*N*`-link` است که در آن *N* نسخه‌ی پروفایل محسوب می‌شود.
...
- [`manifest.nix`](@docroot@/command-ref/files/manifest.nix.md) که توسط [`nix-env`](@docroot@/command-ref/nix-env.md) استفاده می‌شود.
- [`manifest.json`](@docroot@/command-ref/files/manifest.json.md) که توسط [`nix profile`](@docroot@/command-ref/new-cli/nix3-profile.md) (تجربی) استفاده می‌شود.
...
[نصب‌کننده](@docroot@/installation/installing-binary.md) به طور پیش‌فرض این مورد را راه‌اندازی می‌کند، مگر اینکه گزینه [`use-xdg-base-directories`] را فعال کنید.
