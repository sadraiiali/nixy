...
- `--remove-all` / `-r`
  ...
  ابتدا تمام بسته‌های ازپیش‌نصب‌شده را حذف کنید. این کار معادل اجرای اولیه `nix-env --uninstall '.*'` است، با این تفاوت که همه‌چیز در یک تراکنش واحد انجام می‌شود.
...
توجه داشته باشید که نسخه‌ی ازپیش‌نصب‌شده حذف می‌شود، زیرا گزینه‌ی `--preserve-installed` مشخص نشده بود.

برای نصب یک نسخه‌ی دلخواه:
...
"i686-linux";}).subversionWithJava) (import ./foo.nix)` ارزیابی می‌شود؛ در نتیجه صفت `subversionWithJava` را از مجموعه‌ی بازگردانده‌شده با فراخوانی تابع تعریف‌شده در `./foo.nix` انتخاب می‌کند.
