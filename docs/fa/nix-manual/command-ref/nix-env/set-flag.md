...
- صفت `active` را می‌توان روی `false` تنظیم کرد تا بسته «غیرفعال» شود. یعنی هیچ پیوند نمادینی (symlink) به فایل‌های بسته ایجاد نخواهد شد، اما بسته همچنان بخشی از پروفایل باقی می‌ماند (بنابراین توسط جمع‌کننده‌ی زباله (garbage collector) جمع‌آوری نمی‌شود). این صفت را می‌توان مجدداً روی `true` تنظیم کرد تا بسته دوباره فعال شود.
