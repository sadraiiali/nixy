...
> **توجه**
>
> هنگام ارسال یک عبارت ساده‌ی Nix در یک شِبَنگ (shebang) مربوط به nix-shell، باید از نقل‌قول‌های تکی یا جفت (`'`, `"`) استفاده کنید.

در نهایت، با استفاده از ادغام چندین شِبَنگ nix-shell، اسکریپت Haskell زیر از یک شاخه‌ی خاص از Nixpkgs/NixOS (شاخه پایدار 20.03) استفاده می‌کند:
