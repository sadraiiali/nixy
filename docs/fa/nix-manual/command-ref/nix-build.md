...
[`nix-instantiate`](./nix-instantiate.md) (برای ترجمه یک عبارت Nix سطح بالا به یک [store derivation] سطح پایین) و [`nix-store
--realise`](@docroot@/command-ref/nix-store/realise.md) (برای ساخت store derivation) است.

[store derivation]: @docroot@/glossary.md#gloss-store-derivation

> **هشدار**
>
> نتیجه ساخت به‌طور خودکار به عنوان ریشه جمع‌کنندهٔ زباله (garbage collector) نیکس ثبت می‌شود. این ریشه با حذف یا تغییر نام پیوند نمادین `result` به‌طور خودکار ناپدید می‌شود. بنابراین نام پیوند نمادین را تغییر ندهید.

# گزینه‌ها

همه گزینه‌هایی که در اینجا ذکر نشده‌اند، به [`nix-store --realise`](./nix-store/realise.md) منتقل می‌شوند، به جز `--arg` و `--attr` / `-A` که به [`nix-instantiate`](./nix-instantiate.md) منتقل می‌شوند.

- <span id="opt-no-out-link">[`--no-out-link`](#opt-no-out-link)<span>

  یک پیوند نمادین به مسیر خروجی ایجاد نکنید. توجه داشته باشید که در نتیجه، خروجی به ریشه‌ای از جمع‌کنندهٔ زباله تبدیل نمی‌شود و بنابراین ممکن است توسط `nix-store --gc` حذف شود.
...
ساخت بسته GNU Hello از آخرین نسخهٔ (Revision) شاخهٔ master از مجموعهٔ بسته‌های نیکس (Nixpkgs):
