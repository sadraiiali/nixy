...
> Source: [https://nix.dev/manual/nix/stable/command-ref/new-cli/nix3-help-stores.html](@docroot@/command-ref/new-cli/nix3-help-stores.md)
...
This is abstract, just some way to read the metadata of lower store [store objects](https://nix.dev/manual/nix/2.34/store/store-object).
...
The lower store's metadata is considered part of the lower store, just as the store's [file system objects](https://nix.dev/manual/nix/2.34/store/file-system-object) that appear in the store directory are.
- **Upper almost-store**:
...
This contains additional [store objects](https://nix.dev/manual/nix/2.34/store/store-object)
(or, strictly speaking, their [file system objects](https://nix.dev/manual/nix/2.34/store/file-system-object) that the local overlay store will extend the lower store with).
...
This contains the metadata of all of the upper layer [store objects](https://nix.dev/manual/nix/2.34/store/store-object) (everything beyond their file system objects), and also duplicate copies of some lower layer store object's metadata.
The duplication is so the metadata for the [closure](https://nix.dev/manual/nix/2.34/glossary#gloss-closure) of upper layer [store objects](https://nix.dev/manual/nix/2.34/store/store-object) can be found entirely within the upper layer.
...
- `lower-store`
[Store URL](https://nix.dev/manual/nix/2.34/command-ref/new-cli/nix3-help-stores#store-url-format)
...
>
> To use this store, make sure the
> [`mounted-ssh-store` experimental feature](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-mounted-ssh-store)
> is enabled.
> For example, include the following in [`nix.conf`](https://nix.dev/manual/nix/2.34/command-ref/conf-file):
>
> ```
...
This store shares many idioms with the [HTTP Binary Cache Store](https://nix.dev/manual/nix/2.34/store/types/http-binary-cache-store).

For AWS S3, the binary cache URL for a bucket named `example-nix-cache` will be exactly [s3://example-nix-cache](s3://example-nix-cache).
...
> [https://example-nix-cache.s3.amazonaws.com](https://example-nix-cache.s3.amazonaws.com) instead of [s3://example-nix-cache](s3://example-nix-cache).
...
With bucket policies and authentication set up as described above, uploading works via [`nix copy`](https://nix.dev/manual/nix/2.34/command-ref/new-cli/nix3-copy) (experimental).
