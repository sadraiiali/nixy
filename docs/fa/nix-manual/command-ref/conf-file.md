# nix.conf - Nix 2.34.9 Reference Manual

> Source: [https://nix.dev/manual/nix/stable/command-ref/conf-file.html](@docroot@/command-ref/conf-file.md)
...
1. The system-wide configuration file `sysconfdir/nix/nix.conf` (i.e. `/etc/nix/nix.conf` on most systems), or `$NIX_CONF_DIR/nix.conf` if [`NIX_CONF_DIR`](https://nix.dev/manual/nix/2.34/command-ref/env-common#env-NIX_CONF_DIR) is set. 
 Values loaded in this file are not forwarded to the Nix daemon.
The client assumes that the daemon has already loaded them.
2. If [`NIX_USER_CONF_FILES`](https://nix.dev/manual/nix/2.34/command-ref/env-common#env-NIX_USER_CONF_FILES) is set, then each path separated by `:` will be loaded in reverse order. 
 Otherwise it will look for `nix/nix.conf` files in `XDG_CONFIG_DIRS` and [`XDG_CONFIG_HOME`](https://nix.dev/manual/nix/2.34/command-ref/env-common#env-XDG_CONFIG_HOME).
...
3. If [`NIX_CONFIG`](https://nix.dev/manual/nix/2.34/command-ref/env-common#env-NIX_CONFIG) is set, its contents are treated as the contents of a configuration file.
...
- <a id="conf-abort-on-warn"></a><a id="conf-abort-on-warn"></a>`abort-on-warn` 
 If set to true, [`builtins.warn`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-warn) throws an error when logging a warning. 
...
> [`flakes` experimental feature](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-flakes)
> is enabled.
...
- <a id="conf-allow-import-from-derivation"></a><a id="conf-allow-import-from-derivation"></a>`allow-import-from-derivation` 
 By default, Nix allows [Import from Derivation](https://nix.dev/manual/nix/2.34/language/import-from-derivation). 
...
``` 
 The [Nix C++ API documentation](https://nix.dev/manual/nix/2.34/development/documentation#api-documentation) has more details on evaluator internals.
...
- <a id="conf-always-allow-substitutes"></a><a id="conf-always-allow-substitutes"></a>`always-allow-substitutes` 
 If set to `true`, Nix ignores the [`allowSubstitutes`](https://nix.dev/manual/nix/2.34/language/advanced-attributes) attribute in derivations and always attempt to use available substituters. 
 **Default:** `false`
...
>
> To change this setting, make sure the
> [`auto-allocate-uids` experimental feature](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-auto-allocate-uids)
> is enabled.
> For example, include the following in [`nix.conf`](https://nix.dev/manual/nix/2.34/command-ref/conf-file):
>
> ```
...
- <a id="conf-build-dir"></a><a id="conf-build-dir"></a>`build-dir` 
 ...
 See also the per-store [`build-dir`](https://nix.dev/manual/nix/2.34/store/types/local-store#store-local-store-build-dir) setting. 
 **Default:** ``
...
- <a id="conf-builders"></a><a id="conf-builders"></a>`builders` 
 ...
 In addition to the [usual ways of setting configuration options](https://nix.dev/manual/nix/2.34/command-ref/conf-file), the value can be read from a file by prefixing its absolute path with `@`. 
 > **Example**
>
...
>
> If you can’t or don’t want to configure `root` to be able to access the remote machine, set `store` to any [local store](https://nix.dev/manual/nix/2.34/store/types/local-store), e.g. by passing `--store /tmp` to the command on the local machine. 
...
The hostname may be an alias defined in `~/.ssh/config`.
  2. A comma-separated list of [Nix system types](https://nix.dev/manual/nix/2.34/development/building#system-type).
...
If there are multiple machines of the right type, Nix prefers the fastest, taking load into account.
 ...
 A machine is only used to build a derivation if all the features in the derivation's [`requiredSystemFeatures`](https://nix.dev/manual/nix/2.34/language/advanced-attributes#adv-attr-requiredSystemFeatures) attribute are supported by that machine.
  7. A comma-separated list of required system features. 
 A machine is only used to build a derivation if all of the machine’s required features appear in the derivation’s [`requiredSystemFeatures`](https://nix.dev/manual/nix/2.34/language/advanced-attributes#adv-attr-requiredSystemFeatures) attribute.
...
- <a id="conf-cores"></a><a id="conf-cores"></a>`cores` 
 Sets the value of the `NIX_BUILD_CORES` environment variable in the [invocation of the `builder` executable](https://nix.dev/manual/nix/2.34/store/building#builder-execution) of a derivation.
...
enter the debugger like [`builtins.break`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-break): 
 
 This is useful for debugging warnings in third-party Nix code. 
 **Default:** `false`
  - [`builtins.trace`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-trace)
  - [`builtins.traceVerbose`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-traceVerbose) 
if `trace-verbose` is set to true.
  - [`builtins.warn`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-warn)
- <a id="conf-debugger-on-warn"></a><a id="conf-debugger-on-warn"></a>`debugger-on-warn` 
 If set to true and the `--debugger` flag is given, [`builtins.warn`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-warn)
will enter the debugger like [`builtins.break`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-break). 
 This is useful for debugging warnings in third-party Nix code. 
 Use `debugger-on-trace` to also enter the debugger on legacy warnings that are logged with [`builtins.trace`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-trace). 
 **Default:** `false`
...
as a right operand in an [attribute set update expression](https://nix.dev/manual/nix/2.34/language/operators#update),
...
- <a id="conf-eval-profiler"></a><a id="conf-eval-profiler"></a>`eval-profiler` 
 ...
 See [Using the `eval-profiler`](https://nix.dev/manual/nix/2.34/advanced-topics/eval-profiler). 
 **Default:** `disabled`
...
- <a id="conf-eval-system"></a><a id="conf-eval-system"></a>`eval-system` 
 This option defines
[`builtins.currentSystem`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-currentSystem)
...
``` 
 ...
 Experimental features are [further documented in the manual](https://nix.dev/manual/nix/2.34/development/experimental-features). 
 **Default:** *empty*
  - [`auto-allocate-uids`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-auto-allocate-uids)
  - [`blake3-hashes`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-blake3-hashes)
  - [`ca-derivations`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-ca-derivations)
  - [`cgroups`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-cgroups)
  - [`configurable-impure-env`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-configurable-impure-env)
  - [`daemon-trust-override`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-daemon-trust-override)
  - [`dynamic-derivations`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-dynamic-derivations)
  - [`external-builders`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-external-builders)
  - [`fetch-closure`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-fetch-closure)
  - [`fetch-tree`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-fetch-tree)
  - [`flakes`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-flakes)
  - [`git-hashing`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-git-hashing)
  - [`impure-derivations`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-impure-derivations)
  - [`local-overlay-store`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-local-overlay-store)
  - [`mounted-ssh-store`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-mounted-ssh-store)
  - [`nix-command`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-nix-command)
  - [`parse-toml-timestamps`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-parse-toml-timestamps)
  - [`pipe-operators`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-pipe-operators)
  - [`read-only-local-store`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-read-only-local-store)
  - [`recursive-nix`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-recursive-nix)
  - [`verified-fetches`](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-verified-fetches)
...
- <a id="conf-extra-platforms"></a><a id="conf-extra-platforms"></a>`extra-platforms` 
 ...
 Nix only builds a given [store derivation](https://nix.dev/manual/nix/2.34/glossary#gloss-store-derivation) locally when its `system` attribute equals any of the values specified here or in the `system` option. 
...
>
> To change this setting, make sure the
> [`configurable-impure-env` experimental feature](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-configurable-impure-env)
> is enabled.
> For example, include the following in [`nix.conf`](https://nix.dev/manual/nix/2.34/command-ref/conf-file):
>
> ```
...
is passed to the builder if they are listed in [`impureEnvVars`](https://nix.dev/manual/nix/2.34/language/advanced-attributes#adv-attr-impureEnvVars). 
...
This doesn't affect derivations that have [`preferLocalBuild = true`](https://nix.dev/manual/nix/2.34/language/advanced-attributes#adv-attr-preferLocalBuild), which are always built locally. 
 > **Note**
>
...
- <a id="conf-nix-path"></a><a id="conf-nix-path"></a>`nix-path` 
 List of search paths to use for [lookup path](https://nix.dev/manual/nix/2.34/language/constructs/lookup-path) resolution.
This setting determines the value of
[`builtins.nixPath`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-nixPath) and can be used with [`builtins.findFile`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-findFile). 
 
 If the respective paths are accessible, the default values are: 
 
 These files are set up by the [Nix installer](https://nix.dev/manual/nix/2.34/installation/installing-binary).
See [`NIX_STATE_DIR`](https://nix.dev/manual/nix/2.34/command-ref/env-common#env-NIX_STATE_DIR) for details on the environment variable. 
 > **Note**
>
> If [restricted evaluation](https://nix.dev/manual/nix/2.34/command-ref/conf-file#conf-restrict-eval) is enabled, the default value is empty.
>
> If pure evaluation is enabled, `builtins.nixPath` *always* evaluates to the empty list `[ ]`. 
 **Default:** *machine-specific*
  - The configuration setting is overridden by the [`NIX_PATH`](https://nix.dev/manual/nix/2.34/command-ref/env-common#env-NIX_PATH) 
environment variable.
  - `NIX_PATH` is overridden by [specifying the setting as the command line flag](https://nix.dev/manual/nix/2.34/command-ref/conf-file#command-line-flags) `--nix-path` .
  - Any current value is extended by the [`-I` option](https://nix.dev/manual/nix/2.34/command-ref/opt-common#opt-I) or `--extra-nix-path` .
  - `$HOME/.nix-defexpr/channels` 
 The [user channel link](https://nix.dev/manual/nix/2.34/command-ref/files/default-nix-expression#user-channel-link), pointing to the current state of [channels](https://nix.dev/manual/nix/2.34/command-ref/files/channels) for the current user.
...
- <a id="conf-nix-shell-always-looks-for-shell-nix"></a><a id="conf-nix-shell-always-looks-for-shell-nix"></a>`nix-shell-always-looks-for-shell-nix` 
 Before Nix 2.24, [`nix-shell`](https://nix.dev/manual/nix/2.34/command-ref/nix-shell) would only look at `shell.nix` if it was in the working directory - when no file was specified. 
...
- <a id="conf-nix-shell-shebang-arguments-relative-to-script"></a><a id="conf-nix-shell-shebang-arguments-relative-to-script"></a>`nix-shell-shebang-arguments-relative-to-script` 
 ...
 Since Nix 2.24, `nix-shell` resolves these paths in a manner that is relative to the [base directory](https://nix.dev/manual/nix/2.34/glossary#gloss-base-directory), defined as the script's directory. 
 You may set this to `false` to temporarily revert to the behavior of Nix 2.23 and older. 
 Using this setting is not recommended.
It will be deprecated and removed. 
 **Default:** `true`
 ...
 **Default:** `false`
  ...
  - Disable impure constants:
    - [`builtins.currentSystem`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-currentSystem)
    - [`builtins.currentTime`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-currentTime)
    - [`builtins.nixPath`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-nixPath)
    - [`builtins.storePath`](https://nix.dev/manual/nix/2.34/language/builtins#builtins-storePath)
...
[`allowed-uris`](https://nix.dev/manual/nix/2.34/command-ref/conf-file#conf-allowed-uris). 
 **Default:** `false`
...
See the
[Store Types](https://nix.dev/manual/nix/2.34/store/types/)
...
- <a id="conf-substituters"></a><a id="conf-substituters"></a>`substituters` 
 A list of [URLs of Nix stores](https://nix.dev/manual/nix/2.34/store/types/#store-url-format) to be used as substituters, separated by whitespace.
A substituter is an additional [store](https://nix.dev/manual/nix/2.34/glossary#gloss-store) from which Nix can obtain [store objects](https://nix.dev/manual/nix/2.34/store/store-object) instead of building them. 
...
Nix only builds a given [store derivation](https://nix.dev/manual/nix/2.34/glossary#gloss-store-derivation) locally when its `system` attribute equals any of the values specified here or in `extra-platforms`. 
...
- <a id="conf-system-features"></a><a id="conf-system-features"></a>`system-features` 
 ...
 This complements the `system` and `extra-platforms` configuration options and the corresponding [`system`](https://nix.dev/manual/nix/2.34/language/derivations#attr-system) attribute on derivations. 
 A derivation can require system features in the [`requiredSystemFeatures` attribute](https://nix.dev/manual/nix/2.34/language/advanced-attributes#adv-attr-requiredSystemFeatures), and the machine to build the derivation must have them. 
 ...
 **Default:** *machine-specific*
 ...
 These historical pseudo-features are always enabled for backwards compatibility, as they are used in Nixpkgs to route Hydra builds to specific machines.
  - `ca-derivations` 
 Included by default if the [`ca-derivations` experimental feature](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-ca-derivations) is enabled. 
 This system feature is implicitly required by derivations with the [`__contentAddressed` attribute](https://nix.dev/manual/nix/2.34/language/advanced-attributes#adv-attr-__contentAddressed).
  - `recursive-nix` 
 Included by default if the [`recursive-nix` experimental feature](https://nix.dev/manual/nix/2.34/development/experimental-features#xp-feature-recursive-nix) is enabled.
  - `uid-range` 
 ...
 **Deprecated alias:** `binary-cache-public-keys`
  ...
  - the store object is [content-addressed](https://nix.dev/manual/nix/2.34/glossary#gloss-content-addressed-store-object)
- <a id="conf-trusted-substituters"></a><a id="conf-trusted-substituters"></a>`trusted-substituters` 
 A list of [Nix store URLs](https://nix.dev/manual/nix/2.34/store/types/#store-url-format), separated by whitespace.
...
The environment variables used to implement this are documented in the [Environment Variables section](https://nix.dev/manual/nix/2.34/command-ref/env-common). 
 > **Warning**
...
| `~/.nix-channels` | `$XDG_STATE_HOME/nix/channels` | 
 If you already have Nix installed and are using [profiles](https://nix.dev/manual/nix/2.34/package-management/profiles) or [channels](https://nix.dev/manual/nix/2.34/command-ref/nix-channel), you should migrate manually when you enable this option.
