...
برای جزئیات کامل، مستندات [فایل پیکربندی](@docroot@/command-ref/conf-file.md#configuration-file) را ببینید.
...
برای جزئیات، به صفحه‌ی راهنمای `mount 8` مراجعه کنید.

- <span id="env-NIX_STORE_DIR">[`NIX_STORE_DIR`](#env-NIX_STORE_DIR)</span>

  محل انبار Nix را بازنویسی می‌کند.
  در یونیکس، مقدار پیش‌فرض `/nix/store` است که در زمان کامپایل توسط گزینه‌ی ساخت `libstore:store-dir` تعیین می‌شود.
  در ویندوز، هیچ تنظیماتی در زمان کامپایل وجود ندارد؛ مقدار پیش‌فرض `%PROGRAMDATA%\nix\store` است که در زمان اجرا با استفاده از [پوشه‌ی شناخته‌شده‌ی `%PROGRAMDATA%`](#known-folders) تعیین می‌شود.
...
- <span id="env-NIX_LOG_DIR">[`NIX_LOG_DIR`](#env-NIX_LOG_DIR)</span>
  ...
  محل پوشه‌ی گزارش‌های Nix را بازنویسی می‌کند.
  در یونیکس، مقدار پیش‌فرض `/nix/var/log/nix` است که در زمان کامپایل توسط گزینه‌ی ساخت `log-dir` تعیین می‌شود.
  در ویندوز، هیچ تنظیماتی در زمان کامپایل وجود ندارد؛ مقدار پیش‌فرض `%PROGRAMDATA%\nix\log` است که در زمان اجرا با استفاده از [پوشه‌ی شناخته‌شده‌ی `%PROGRAMDATA%`](#known-folders) تعیین می‌شود.
...
- <span id="env-NIX_STATE_DIR">[`NIX_STATE_DIR`](#env-NIX_STATE_DIR)</span>
  ...
  محل پوشه‌ی وضعیت Nix را بازنویسی می‌کند.
  در یونیکس، مقدار پیش‌فرض `${localstatedir}/nix` است که در آن `localstatedir` یک گزینه‌ی ساخت در زمان کامپایل است که به طور پیش‌فرض `/nix/var` می‌باشد.
  در ویندوز، هیچ تنظیماتی در زمان کامپایل وجود ندارد؛ مقدار پیش‌فرض `%PROGRAMDATA%\nix\state` است که در زمان اجرا با استفاده از [پوشه‌ی شناخته‌شده‌ی `%PROGRAMDATA%`](#known-folders) تعیین می‌شود.
...
- <span id="env-NIX_DAEMON_SOCKET_PATH">[`NIX_DAEMON_SOCKET_PATH`](#env-NIX_DAEMON_SOCKET_PATH)</span>
  ...
  برای جزئیات درباره‌ی نحوه حل مسیر سوکت، به مستندات [انبار دائم محلی][Local Daemon Store] مراجعه کنید.
...
- <span id="env-NIX_REMOTE">[`NIX_REMOTE`](#env-NIX_REMOTE)</span>
  ...
  تنظیمات [`store`](@docroot@/command-ref/conf-file.md#conf-store) را بازنویسی می‌کند.
...
اگر Nix به گونه‌ای پیکربندی شده باشد که از جمع‌کننده‌ی زباله Boehm استفاده کند، این متغیر اندازه اولیه‌ی هیپ (heap) را بر حسب بایت تنظیم می‌کند. مقدار پیش‌فرض آن ۳۸۴ مگابایت (MiB) است. تنظیم کردن آن روی یک مقدار پایین، مصرف حافظه را کاهش می‌دهد، اما به دلیل سربار جمع‌آوری زباله (garbage collection)، زمان اجرا را افزایش خواهد داد.

- <span id="env-NIX_PATH">[`NIX_PATH`](#env-NIX_PATH)</span>

  فهرستی جداشده با دو نقطه (کولون) از ورودی‌های مسیر جستجو که برای حل [مسیرهای جستجو](@docroot@/language/constructs/lookup-path.md) استفاده می‌شوند.

  این متغیر محیطی، مقدار [تنظیم پیکربندی `nix-path`](@docroot@/command-ref/conf-file.md#conf-nix-path) را بازنویسی می‌کند.

  می‌توان آن را با استفاده از [گزینه‌ی `-I`](@docroot@/command-ref/opt-common.md#opt-I) گسترش داد.
...
- [پوشه پیکربندی کاربر]{#user-conf-dir}: [`NIX_CONFIG_HOME`]{#env-NIX_CONFIG_HOME}
- پوشه وضعیت کاربر: [`NIX_STATE_HOME`]{#env-NIX_STATE_HOME}
- پوشه کش کاربر: [`NIX_CACHE_HOME`]{#env-NIX_CACHE_HOME}
...
[دستورات جدید Nix](@docroot@/command-ref/new-cli/nix.md) (آزمایشی) به‌طور پیش‌فرض از پوشه‌های مناسب استفاده می‌کنند.
...
### یونیکس: پوشه‌های پایه XDG {#xdg-base-directories}
...
- [`XDG_CONFIG_HOME`]{#env-XDG_CONFIG_HOME} (پیش‌فرض `~/.config`)
- [`XDG_STATE_HOME`]{#env-XDG_STATE_HOME} (پیش‌فرض `~/.local/state`)
- [`XDG_CACHE_HOME`]{#env-XDG_CACHE_HOME} (پیش‌فرض `~/.cache`)
- [`XDG_CONFIG_DIRS`]{#env-XDG_CONFIG_DIRS} (پیش‌فرض `/etc/xdg`)، فهرستی جداشده با دونقطه از پوشه‌های پایه پیکربندی اضافی که پس از `XDG_CONFIG_HOME` جستجو می‌شوند

### ویندوز: پوشه‌های شناخته‌شده {#known-folders}
...
- [`%APPDATA%`]{#env-APPDATA}، داده‌های برنامه رومینگ مختص هر کاربر
- [`%LOCALAPPDATA%`]{#env-LOCALAPPDATA}، داده‌های برنامه محلی مختص هر کاربر
- [`%PROGRAMDATA%`]{#env-PROGRAMDATA}، داده‌های برنامه در سطح سیستم
