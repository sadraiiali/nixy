...
عملکرد `--delete` مسیرهای انبار *paths* را از انبار Nix حذف می‌کند، اما تنها در صورتی که انجام این کار امن باشد؛ یعنی زمانی که مسیر از طریق هیچ‌یک از ریشه‌های جمع‌کنندهٔ زباله قابل دسترسی نباشد. این بدان معناست که شما تنها می‌توانید مسیرهایی را حذف کنید که توسط `nix-store --gc` نیز حذف می‌شوند. بنابراین، `--delete` نسخه هدایت‌شده‌تری از `--gc` است.
