...
`nix-store --query` - نمایش اطلاعات دربارهٔ مسیرهای انبار
...
عملیات `--query` اطلاعات مختلفی را دربارهٔ مسیرهای انبار نمایش می‌دهد. پرس‌وجوها در ادامه شرح داده شده‌اند. حداکثر یک پرس‌وجو را می‌توان مشخص کرد. پرس‌وجوی پیش‌فرض `--outputs` است.
...
چاپ [deriver]ی که برای ساخت مسیرهای انبار *paths* استفاده شده‌است. اگر مسیر فاقد deriver باشد (مثلاً یک فایل سورس باشد)، یا deriver آن ناشناخته باشد (مثلاً در سناریوی استقرار صرفاً باینری)، رشتهٔ `unknown-deriver` چاپ می‌شود.
...
- `--roots`
  ...
  ریشه‌های جمع‌کنندهٔ زباله (garbage collector) که مستقیماً یا غیرمستقیم به مسیرهای انبار *paths* اشاره می‌کنند را چاپ می‌کند.
...
چاپ کردن بسته (وابستگی‌های زمان اجرا) برنامهٔ `svn` در محیط کاربر فعلی:
...
تفاوت این مثال با مثال قبلی در این است که ما بستهٔ کامل (closure) مربوط به derivation (`-qd`) را درخواست می‌کنیم، نه بستهٔ کامل مسیر خروجی حاوی `svn` را.
...
تمام ریشه‌های جمع‌کنندهٔ زباله (garbage collector) را نشان بده که به یک مسیر انبار اشاره می‌کنند که به `svn` وابسته است:
