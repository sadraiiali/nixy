...
- [بسته (closure)]{#gloss-closure}
  ...
  بسته یک مسیر انبار، مجموعه ای از مسیرهای انبار است که مستقیماً یا غیرمستقیم از آن مسیر انبار «قابل دسترسی» هستند؛ یعنی بسته‌ی آن مسیر تحت رابطه‌ی *ارجاعات* است. برای یک بسته، بسته‌ی derivation آن معادل وابستگی‌های زمان ساخت است، در حالی که بسته‌ی [مسیر خروجی] آن معادل وابستگی‌های زمان اجرای آن است. برای استقرار صحیح، لازم است کل بسته‌ها مستقر شوند، در غیر این صورت ممکن است در زمان اجرا فایل‌ها گم شوند. دستور `nix-store --query --requisites` بسته‌های مسیرهای انبار را چاپ می‌کند.

  به عنوان مثال، اگر [شیء انبار] در مسیر `P` شامل یک [ارجاع] به یک شیء انبار در مسیر `Q` باشد، آنگاه `Q` در بسته‌ی `P` قرار دارد. علاوه بر این، اگر `Q` به `R` ارجاع دهد، آنگاه `R` نیز در بسته‌ی `P` قرار دارد.
...
برای اطلاع از نحوه استفاده‌ی Nix از آدرس‌دهی محتوا (content-addressing)، به موارد زیر مراجعه کنید:
  ...
  مقاله‌ی Software Heritage در مورد [*شناسه‌های ذاتی و بیرونی*](https://www.softwareheritage.org/2020/07/09/intrinsic-vs-extrinsic-identifiers) نیز مقدمه‌ی خوبی برای درک ارزش آدرس‌دهی محتوا در مقایسه با سایر طرح‌های ارجاع است.
...
- [derivation]{#gloss-derivation}
  ...
  این sandbox به‌طور پیش‌فرض تنها اجازه‌ی خواندن از اشیای انباری را می‌دهد که به عنوان ورودی مشخص شده‌اند، و تنها اجازه‌ی نوشتن روی [خروجی‌های][output] تعیین‌شده را می‌دهد تا به عنوان [اشیای انبار ثبت شوند](@docroot@/store/building.md#processing-outputs).
...
- [سازنده‌ی derivation (deriver)]{#gloss-deriver}

  آن [derivation انبار] که یک [مسیر خروجی] را تولید کرده‌است.

  سازنده‌ی یک مسیر خروجی را می‌توان با گزینه‌ی `--deriver` در دستور
...
- [قابل‌نصب]{#gloss-installable}
  ...
  برای جزئیات بیشتر درباره‌ی [`دستورات nix`](./command-ref/new-cli/nix.md) (آزمایشی)، به [قابل‌نصب‌ها](./command-ref/new-cli/nix.md#installables) مراجعه کنید.
...
- [بسته (package)]{#package}
  ...
  - صفاتی که به فایل‌های یک بسته اشاره دارند، معمولاً در قالب [خروجی‌های درایویشن](#gloss-output)، صفاتی با فراداده، مانند اطلاعاتی درباره‌ی نحوه استفاده از بسته.
...
- [قابل‌دسترس]{#gloss-reachable}
  ...
  یک مسیر انبار `Q` از مسیر انبار دیگری چون `P` قابل‌دسترس است اگر `Q` در *بستار* رابطه‌ی *ارجاعات* باشد.
...
- [محقق‌سازی (realise)]{#gloss-realise}، تحقق
  ...
  برای توضیحات دقیق درباره‌ی الگوریتم، به [`nix-store --realise`](@docroot@/command-ref/nix-store/realise.md) مراجعه کنید.
