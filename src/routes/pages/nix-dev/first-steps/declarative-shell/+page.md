# <a id="declarative-reproducible-envs"></a> محیط‌های شل اعلامی با `shell.nix`
...
در آموزش [ad-hoc-envs](/pages/nix-dev/tutorials/first-steps/ad-hoc-shell-environments)، یاد گرفتید که چگونه با استفاده از `nix-shell -p` به روش دستوری (imperative) محیط‌های شل ایجاد کنید.
این کار زمانی که می‌خواهید بدون نصب دائم ابزارها به سرعت به آن‌ها دسترسی داشته باشید عالی است.
همچنین یاد گرفتید که چگونه آن دستور را با نسخه‌ای خاص از Nixpkgs با استفاده از یک کامیت گیت به عنوان آرگومان اجرا کنید تا همان محیط قبلی بازآفرینی شود.

در این آموزش، بررسی خواهیم کرد که چگونه با استفاده از یک پیکربندی اعلانی (declarative) در یک *فایل Nix*، محیط‌های شل بازتولیدپذیر ایجاد کنیم.
...
- آشنایی با شل یونیکس
- درک مقدماتی از [زبان Nix](/pages/nix-dev/tutorials/nix-language)
...
**توضیحات تفصیلی**
ما از نسخه‌ای از [Nixpkgs سنجاق‌شده به یک شاخه انتشار](/pages/nix-dev/reference/pinning-nixpkgs) استفاده می‌کنیم.
اگر آموزش [ad-hoc-envs](/pages/nix-dev/tutorials/first-steps/ad-hoc-shell-environments) را دنبال کرده‌اید و نمی‌خواهید تمام وابستگی‌ها را دوباره بارگیری کنید، دقیقاً همان ریویژن (revision) مشخص‌شده در بخش [towards-reproducibility](/pages/nix-dev/tutorials/first-steps/ad-hoc-shell-environments) را تعیین کنید:
...
**نکته‌ی حاشیه‌ای درباره‌ی `mkShell`**
...
- [reading-nix-language](/pages/nix-dev/tutorials/nix-language)
- [automatic-direnv](/pages/nix-dev/guides/recipes/direnv)
- [sharing-dependencies](/pages/nix-dev/guides/recipes/sharing-dependencies)
