# <a id="automatic-direnv"></a> فعال‌سازی خودکار محیط با `direnv`

به‌جای فعال‌سازی دستی محیط برای هر پروژه، می‌توانید هر زمان که وارد پوشه پروژه می‌شوید یا فایل `shell.nix` درون آن را تغییر می‌دهید، یک [محیط‌های اعلامی و قابل بازتولید](/pages/nix-dev/tutorials/first-steps/declarative-shell) را مجدداً بارگیری کنید.
...
دفعه‌ی بعدی که ترمینال خود را باز کرده و وارد پوشه‌ی سطح بالای پروژه خود شوید، `direnv` به طور خودکار شل تعریف‌شده در `shell.nix` را اجرا خواهد کرد.
