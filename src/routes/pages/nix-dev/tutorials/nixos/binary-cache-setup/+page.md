# <a id="setup-http-binary-cache"></a> راه‌اندازی کش باینری HTTP
...
- یک [نصب Nix](/pages/nix-dev/install-nix) کاری روی ماشین محلی خود

- دسترسی SSH به یک ماشین NixOS برای استفاده به عنوان کش

- اگر با NixOS تازه آشنا شده‌اید، درباره‌ی [سیستم ماژول](/pages/nix-dev/tutorials/module-system) بیاموزید و اولین سیستم خود را با [nixos-vms](/pages/nix-dev/tutorials/nixos/nixos-configuration-on-vm) پیکربندی کنید.
...
- (اختیاری) یک آی‌پی عمومی و دامین DNS
  ...
  برای استقرار پیکربندی NixOS خود، آموزش [provisioning-remote-machines-tutorial](/pages/nix-dev/tutorials/nixos/provisioning-remote-machines) را دنبال کنید.
...
> **مهم**
>
> [custom-binary-cache](/pages/nix-dev/guides/recipes/add-binary-cache) با استفاده از `cache-public-key.pem` روی ماشین محلی خود.
...
### <a id="https-binary-cache"></a> ارائه کش باینری از طریق HTTPS
...
- [custom-binary-cache](/pages/nix-dev/guides/recipes/add-binary-cache) با استفاده از نام هاست کش باینری و کلید عمومی تولید‌شده
- [post-build-hooks](/pages/nix-dev/guides/recipes/post-build-hook) برای بارگذاری اشیاء انبار به کش باینری
- [distributed-build-setup-tutorial](/pages/nix-dev/tutorials/nixos/distributed-builds-setup)
