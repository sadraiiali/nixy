# <a id="integration-testing-vms"></a> تست یکپارچه‌سازی با ماشین‌های مجازی NixOS
...
- یک [نصب Nix](/pages/nix-dev/install-nix) فعال روی لینوکس یا [NixOS](https://nixos.org/manual/nixos/stable/index.html#sec-installation)
- دانش پایه‌ای از [زبان Nix](/pages/nix-dev/tutorials/nix-language)
- دانش پایه‌ای از [پیکربندی NixOS](/pages/nix-dev/tutorials/nixos/nixos-configuration-on-vm)
...
۱. از یک [نسخهٔ سنجاق‌شده از Nixpkgs](/pages/nix-dev/reference/pinning-nixpkgs) استفاده کنید، و [گزینه‌های پیکربندی و اورلی‌ها را به طور صریح تنظیم کنید](/pages/nix-dev/guides/best-practices) تا از بازنویسی غیرعمدی آن‌ها توسط پیکربندی سراسری جلوگیری شود:
...
1. از آنجا که این مثال تنها از یک ماشین مجازی استفاده می‌کند، نودی که مشخص می‌کنیم به‌سادگی `machine` نامیده می‌شود.
   ...
   به عنوان پیکربندی، از بخش‌های مرتبطِ پیکربندی پیش‌فرض استفاده می‌کنید [که در یک آموزش قبلی از آن استفاده کردیم](/pages/nix-dev/tutorials/nixos/nixos-configuration-on-vm):
...
- اجرای ماشین‌های مجازی تست یکپارچه‌سازی روی ادغام مداوم (CI) نیازمند شتاب‌دهی سخت‌افزاری است، که بسیاری از سرویس‌های ادغام مداوم از آن پشتیبانی نمی‌کنند.
  ...
  برای اجرای ماشین‌های مجازی تست یکپارچه‌سازی در [GitHub Actions](/pages/nix-dev/guides/recipes/continuous-integration-github-actions)، به بخش [نحوه غیرفعال کردن شتاب‌دهی سخت‌افزاری](https://github.com/cachix/install-nix-action#how-do-i-run-nixos-tests) مراجعه کنید.
...
- [nixos-docker-images](/pages/nix-dev/tutorials/nixos/building-and-running-docker-images)
