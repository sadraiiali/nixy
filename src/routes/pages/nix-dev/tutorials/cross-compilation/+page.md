...
- دانش پایه از [زبان Nix](/pages/nix-dev/tutorials/nix-language)
...
مجموعه‌ی بسته‌های نیکس (Nixpkgs) دارای مجموعه‌ای از هاست پلتفرم‌های از‌پیش‌تعریف‌شده برای کامپایل متقاطع به نام `pkgsCross` است.
...
در [آموزش محیط‌های اعلامی و قابل بازتولید](/pages/nix-dev/tutorials/first-steps/declarative-shell)، بررسی کردیم که چگونه Nix به ما کمک می‌کند تا ابزارها و کتابخانه‌های سیستم را برای پروژه‌مان فراهم کنیم.
...
- [کش باینری رسمی](https://cache.nixos.org) تعداد محدودی از باینری‌ها را برای بسته‌هایی که کامپایل متقاطع شده‌اند دارد، بنابراین برای صرفه‌جویی در زمان کامپایل مجدد، [کش باینری و CI خود را با GitHub Actions پیکربندی کنید](/pages/nix-dev/guides/recipes/continuous-integration-github-actions)۔
...
- در حالی که بسیاری از کامپایلرها در Nixpkgs از کامپایل متقاطع پشتیبانی می‌کنند، همه آن‌ها چنین قابلیتی ندارند.
 ...
 [توضیحات تفصیلی درباره نحوه پیاده‌سازی کامپایل متقاطع در Nix](https://nixos.org/manual/nixpkgs/stable/#chap-cross) می‌تواند در رفع آن مسائل کمک کند.
