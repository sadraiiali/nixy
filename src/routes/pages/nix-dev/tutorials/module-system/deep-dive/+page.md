# <a id="module-system-deep-dive"></a> بررسی عمیق سیستم ماژول
...
فایل‌های `map.sh` و `geocode.sh` را در پوشه کاری خود بارگیری کنید.
...
در این حالت، یک دستور شل اختصاص می‌دهید که اسکریپت `map` را در پوشه جاری اجرا می‌کند.
...
ما می‌توانیم این مشکل را با بسته‌بندی اسکریپت خام `map` به وسیله‌ی `pkgs.writeShellApplication` حل کنیم.
...
برای پیاده‌سازی این رفتار، از ابزار `geocode` استفاده خواهید کرد که نام مکان‌ها را به مختصات تبدیل می‌کند.
...
> **نکته**
>
> گزینه‌های ماژول دارا‌ی یک *اولویت* (priority) هستند که به صورت یک عدد صحیح نشان داده می‌شود و تقدم تنظیم گزینه روی یک مقدار خاص را تعیین می‌کند.
