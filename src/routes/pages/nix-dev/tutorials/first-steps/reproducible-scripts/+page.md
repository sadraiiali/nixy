# <a id="reproducible-scripts"></a> اسکریپت‌های تفسیری بازتولیدپذیر
...
- یک [نصب Nix](/pages/nix-dev/install-nix) فعال
- آشنایی با [Bash]
...
این ابزار پارامترهای زیر را که برای مورد استفاده‌ی ما مرتبط هستند می‌پذیرد:

- `-i` مشخص می‌کند که برای تفسیر بقیه‌ی فایل از چه برنامه‌ای استفاده شود
...
جزئیات بیشتر درباره‌ی گزینه‌ها را می‌توانید در [مستندات مرجع `nix-shell`](/pages/nix-manual/command-ref/nix-shell#options) بیابید.
...
- با گزینه `-i`، برنامه‌ی `bash` به عنوان مفسر برای بقیه فایل مشخص می‌شود.

- در این حالت، گزینه‌ی `--pure` فعال می‌شود تا از استفاده‌ی ضمنی اسکریپت از برنامه‌هایی که ممکن است از پیش روی سیستمی که اسکریپت روی آن اجرا می‌شود وجود داشته باشند، جلوگیری کند.

- گزینه‌ی `-p` بسته‌های مورد نیاز برای اجرای اسکریپت را فهرست می‌کند.

 دستور `xml2json` توسط بسته‌ی `python3Packages.xmljson` فراهم می‌شود، در حالی که `bash`، `jq` و `curl` توسط بسته‌هایی با همین نام فراهم می‌شوند.
بسته‌ی `cacert` باید حضور داشته باشد تا احراز هویت SSL کار کند.

> <span class="admonition-kind" data-kind="tip"></span>
>
> **راهنمایی**
>
> برای پیدا کردن بسته‌هایی که برنامه‌ی مورد نیاز شما را فراهم می‌کنند، از [search.nixos.org](https://search.nixos.org/packages) استفاده کنید.
...
- [reading-nix-language](/pages/nix-dev/tutorials/nix-language) برای کسب اطلاعات درباره‌ی زبان Nix که برای اعلام بسته‌ها و پیکربندی‌ها استفاده می‌شود.
- [declarative-reproducible-envs](/pages/nix-dev/tutorials/first-steps/declarative-shell) برای ایجاد محیط‌های شل بازتولیدپذیر با یک فایل پیکربندی اعلانی (declarative).
