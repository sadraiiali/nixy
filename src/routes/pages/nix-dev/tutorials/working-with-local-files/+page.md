# <a id="file-sets-tutorial"></a> کار با فایل‌های محلی
...
شما می‌توانید با استفاده از [`nix repl`](/pages/nix-manual/command-ref/experimental-commands) کتابخانه را بررسی کرده و درباره آن بیاموزید:
...
> یک پوشه محلی درون یک فلیک همیشه به طور *كامل* در انبار نیکس کپی می‌شود، مگر اینکه یک مخزن گیت باشد.
...
اسکریپت `postInstall` ساده‌سازی شده است تا متکی بر این باشد که کدهای منبع به شکل مناسبی از‌پیش‌فیلتر شده باشند:
