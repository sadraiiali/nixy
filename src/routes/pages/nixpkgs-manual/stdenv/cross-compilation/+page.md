# <a id="chap-cross"></a> کامپایل متقاطع

## <a id="sec-cross-intro"></a> مقدمه
...
انواع وابستگی، روابطی را که یک بسته با هر یک از وابستگی‌های متعدی (transitive) خود دارد، توصیف می‌کنند. می‌توانید این‌طور تصور کنید که یک یا چند نوع وابستگی را به هر یک از پارامترهای رسمی در بالای فایل `.nix` یک بسته، و همچنین به همهٔ پارامترهای رسمی *آن‌ها* و الی آخر نسبت می‌دهید. در مقابل، سه‌تایی‌هایی مانند `(foo`
...
و به‌طور خاص برای `aarch64-linux` و بسیاری از پلتفرم‌های دیگر، همهٔ آن‌ها در بستهٔ `qemu` در دسترس هستند؛ به این معنی که می‌توانید به سادگی اجرا کنید:
...
#### <a id="cross-qa-build-c-program-in-build-environment"></a> چه می‌شود اگر سیستم ساخت بستهٔ شما نیاز به ساخت یک برنامه C برای اجرا در محیط ساخت داشته باشد؟
...
#### <a id="cross-testsuite-runs-host-code"></a> مجموعه تست بسته‌ام به اجرای کد پلتفرم میزبان نیاز دارد.
...
#### <a id="cross-meson-runs-host-code"></a> بسته‌ای که از Meson استفاده می‌کند نیاز دارد باینری‌های پلتفرم میزبان را در طول ساخت اجرا کند.
...
> >
> > ## <a id="sec-cross-usage"></a> کامپایل متقاطع بسته‌ها
> >
