...
Everything looks solid and precise. Outputting Persian translation only.# نکات مربوط به پلتفرم <a id="chap-platform-notes"></a>

## <a id="sec-darwin"></a> Darwin (macOS)

`stdenv` در Darwin با بیشتر موارد دیگر در Nixpkgs در چند جنبه کلیدی تفاوت دارد.

##### <a id="sec-darwin-libcxx-deployment-targets"></a> استفاده از یک deployment target جدیدتر
...
#### <a id="sec-darwin-troubleshooting-using-deployment-targets"></a> «مقصد استقرار» (یا حداقل نسخه) چیست؟
...
#### <a id="sec-darwin-troubleshooting-picking-sdk-version"></a> انتخاب یک نسخه SDK
...
#### <a id="sec-darwin-troubleshooting-xcodebuild"></a> بسته به `xcodebuild` نیاز دارد
...
##### <a id="sec-darwin-troubleshooting-xcodebuild-absolute-paths"></a> اصلاح مسیرهای مطلق به `xcodebuild`، `xcrun` و `PlistBuddy`
...
##### <a id="sec-darwin-troubleshooting-install-name-install_name_tool"></a> تنظیم install name با استفاده از `install_name_tool`
