...
### <a id="sec-building-stdenv-package-in-nix-shell"></a> ساخت یک بسته `stdenv` در `nix-shell`
...
ارائه توضیحی درباره این ساختار پیچیده لازم است. در حالت معمولِ `nativeBuildInputs` یا `buildInputs`، آفست هدفِ یک وابستگی، یکی بیشتر از آفست میزبان است: `t = h + 1`. این بدین معناست که:
...
##### <a id="var-stdenv-depsBuildTarget"></a> `depsBuildTarget`
...
Translation 18:
##### <a id="var-stdenv-preConfigurePhases"></a> `preConfigurePhases`
...
##### <a id="var-stdenv-prefixKey"></a> `prefixKey`
...
##### <a id="var-stdenv-makeFlagsArray"></a> `makeFlagsArray`
...
##### <a id="var-stdenv-buildFlags"></a> `buildFlags` / `buildFlagsArray`
...
##### <a id="var-stdenv-preBuild"></a> `preBuild`

قلابی که در ابتدای فاز ساخت اجرا می‌شود.

##### <a id="var-stdenv-postBuild"></a> `postBuild`
...
##### <a id="var-stdenv-stripAllFlags"></a> `stripAllFlags`
...
##### <a id="var-stdenv-stripExclude"></a> `stripExclude`
...
##### <a id="var-stdenv-dontPatchELF"></a> `dontPatchELF`
##### <a id="var-stdenv-dontPatchShebangs"></a> `dontPatchShebangs`
##### `dontPruneLibtool`
...
### <a id="fun-runHook"></a> `runHook` \&lt;hook\&gt;
...
### <a id="fun-substituteInPlace"></a> `substituteInPlace` \&lt;multiple files\&gt; \&lt;subs\&gt;
...
### <a id="fun-wrapProgram"></a> `wrapProgram` \&lt;executable\&gt; \&lt;makeWrapperArgs\&gt;
...
### <a id="fun-appendToVar"></a> `appendToVar` \&lt;variableName\&gt; \&lt;elements...\&gt;
...
جانبی)
    ...
    - Anchors and markdowns intact (`## Package setup hooks <a id="ssec-setup-hooks"></a>`, `*relative*`, `[by convention...]`).
...
### <a id="move-docs.sh"></a> `move-docs.sh`
...
##### <a id="patch-shebangs.sh-invocation-flags"></a> پرچم‌ها
...
##### <a id="patch-shebangs.sh-invocation-examples"></a> مثال‌ها
...
#### <a id="stackprotector"></a> `stackprotector`
...
#### <a id="fortify"></a> `fortify`

گزینه‌های کامپایلر `-O2 -D_FORTIFY_SOURCE=2` را اضافه می‌کند. در طول تولید کد، کامپایلر اطلاعات زیادی درباره اندازه‌های بافر (در صورت امکان) می‌داند و تلاش می‌کند فراخوانی‌های ناامن توابع بافر با طول نامحدود
...
#### <a id="strictoverflow"></a> `strictoverflow`
...
#### <a id="zerocallusedregs"></a> `zerocallusedregs`
...
#### <a id="glibcxxassertions"></a> `glibcxxassertions`
