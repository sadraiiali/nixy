<script lang="ts">
	...
	import navData from '$lib/nixpkgs-manual-nav.json';
	...
	const sectionLabel: Record<string, string> = {
		root: 'شروع',
		'using-nixpkgs': 'استفاده از Nixpkgs',
		using: 'استفاده',
		lib: 'کتابخانه',
		stdenv: 'stdenv',
		toolchains: 'toolchainها',
		'build-helpers': 'ابزارهای ساخت',
		modules: 'ماژول‌ها',
		'module-system': 'سیستم ماژول',
		development: 'توسعه',
		contributing: 'مشارکت',
	...
	function sectionBase(sec: string) {
		return sec === 'root' ? '/pages/nixpkgs-manual' : `/pages/nixpkgs-manual/${sec}`;
	}
	...
	const currentTitle = $derived(
		currentPage ? cleanTitle(currentPage.title) : 'راهنمای Nixpkgs'
	);

	const currentSection = $derived.by(() => {
		if (currentPage?.section) return currentPage.section || 'root';
		const path = norm(page.url.pathname);
		const rest = path.replace(/^\/pages\/nixpkgs-manual\/?/, '');
		if (!rest) return 'root';
	...
	function sourceFromPage(p: NavPage | null): { href: string; label: string } {
		...
		const path = norm(page.url.pathname).replace(/^\/pages\/nixpkgs-manual\/?/, '');
...
<svelte:head>
	<title>{currentTitle} · Nixpkgs · نیکسی</title>
	<meta name="description" content={`${currentTitle} — راهنمای Nixpkgs (فارسی) در نیکسی.`} />
	<meta property="og:title" content={`${currentTitle} · Nixpkgs · نیکسی`} />
	<meta property="og:description" content={`${currentTitle} — راهنمای Nixpkgs (فارسی) در نیکسی.`} />
	<meta name="twitter:title" content={`${currentTitle} · Nixpkgs · نیکسی`} />
</svelte:head>
...
<div class="nd" class:nd--nav-open={navOpen}>
	...
	<aside
		...
		aria-label="فهرست راهنمای Nixpkgs"
	>
		<div class="nd-nav__head">
			<p class="nd-nav__brand">
				<a href="/pages/nixpkgs-manual" onclick={closeNav}>راهنمای Nixpkgs</a>
