# <a id="python"></a> پایتون

## <a id="reference"></a> مرجع

### <a id="interpreters"></a> مفسرها
...
#### <a id="missing-tkinter-module-standard-library"></a> نبود کتابخانه استاندارد ماژول `tkinter`

برای کاهش اندازه بستار، ماژول `Tkinter`/`tkinter` به عنوان یک بسته جداگانه، یعنی `pythonPackages.tkinter` در دسترس است.

#### <a id="attributes-on-interpreters-packages"></a> صفات در بسته‌های مفسرها

هر مفسر دارای صفات زیر است:

- `libPrefix`. نام پوشه در `${'{'}'{'{'}'{'}'}python{'{'}'{'}'}'{'}'}/lib/` برای مفسر مربوطه.
- `interpreter`. نام مستعار برای `${'{'}'{'{'}'{'}'}python{'{'}'{'}'}'{'}'}/bin/$`.
...
به منظور حفظ سازگاری، هنگامی که متغیر شل `makeWrapperArgs` به صورت یک رشتهٔ جداشده با فاصله (به جای یک آرایهٔ Bash) در اسکریپت ساخت مشخص شود، محتوای رشته پیش از الحاق به دستور `wrapProgram` توسط Bash گسترش می‌یابد. با این
...
#### <a id="buildpythonapplication-function"></a> تابع `buildPythonApplication`
...
#### <a id="topythonapplication-function"></a> تابع `toPythonApplication`
...
#### <a id="topythonmodule-function"></a> تابع `toPythonModule`
...
#### <a id="mkpythonmetapackage-function"></a> تابع `mkPythonMetaPackage`
...
#### <a id="mkpythoneditablepackage-function"></a> تابع `mkPythonEditablePackage`
...
#### <a id="python.buildenv-function"></a> تابع `python.buildEnv`

محیط‌های پایتون را می‌توان با استفاده از تابع سطح پایین `pkgs.buildEnv` ایجاد کرد.
این مثال نحوهٔ ایجاد محیطی را نشان می‌دهد که دارای چارچوب وب Pyramid است.
...
##### <a id="python.buildenv-arguments"></a> آرگومان‌های `python.buildEnv`
...
### <a id="using-python"></a> Using Python
...
### <a id="developing-with-python"></a> توسعه با Python
...
#### <a id="python-library-packages-in-nixpkgs"></a> بسته‌های کتابخانه‌ای Python در Nixpkgs
...
#### <a id="testing-python-packages"></a> تست بسته‌های Python
...
#### <a id="using-pythonimportscheck"></a> استفاده از pythonImportsCheck
...
گزینهٔ دیگر، پاس دادن `true` است که تمام وابستگی‌ها را تسهیل/حذف می‌کند؛ برای مثال:
...
#### <a id="using-sphinxhook"></a> استفاده از sphinxHook
...
این قلاب، در صورت وجود خروجی `doc`، فرآوردهٔ ساخت را به‌طور خودکار ساخته و در آن نصب می‌کند. همچنین یک تغییر مسیر خودکار برای فرآورده‌های ساختِ سازنده (Builder) `man` به سمت هدف `man` فراهم می‌کند.
...
### <a id="how-to-solve-circular-dependencies"></a> How to solve circular dependencies?
...
بستهٔ درخواست‌شده `blaze` به `pandas` وابسته است که خود به `scipy` وابسته است.

اگر می‌خواهید تمام Nixpkgs از تغییرات شما استفاده کند، همان‌طور که در این راهنما توضیح داده شده است، می‌توانید از `overlays` استفاده کنید. در مثال زیر، یک `inkscape` را با استفاده از نسخهٔ متفاوتی از `numpy` می‌سازیم.
...
### <a id="python-setup.py-bdist_wheel-cannot-create-.whl"></a> دستور `python setup.py bdist_wheel` نمی‌تواند .whl را ایجاد کند
...
### <a id="install_data-data_files-problems"></a> مشکلات `install_data` / `data_files`
...
### <a id="rationale-of-non-existent-global-site-packages"></a> دلیل عدم وجود site-packages سراسری

در بیشتر سیستم‌عامل‌ها یک `site-packages` سراسری نگه‌داری می‌شود. با این حال، اگر بخواهید چندین نسخهٔ پایتون را اجرا کنید یا نسخه‌های متعددی از کتابخانه‌های خاصی را برای پروژه‌های خود داشته باشید، این امر مشکل‌ساز می‌شود. به‌طور کلی، شما چنین مشکلاتی را با ایجاد محیط‌های مجازی با استفاده از `virtualenv` حل می‌کنید.
...
So:
با این حال، این بسته‌ها به‌صورت محلی در پوشه `virtualenv` ذخیره‌شده در حافظهٔ پنهان خواهند ماند و
...
### <a id="how-to-override-a-python-package-using-overlays"></a> چگونه یک بسته پایتون را با استفاده از اورلی‌ها بازنشانی کنیم؟
...
### <a id="how-to-override-a-python-package-for-all-python-versions-using-extensions"></a> چگونه یک بسته Python را برای تمام نسخه‌های Python با استفاده از افزونه‌ها بازنشانی کنیم؟
...
### <a id="how-to-use-intels-mkl-with-numpy-and-scipy"></a> چگونه از MKL اینتل همراه با numpy و scipy استفاده کنیم؟
...
### <a id="python-optional-dependencies"></a> چگونه وابستگی‌های اختیاری را اضافه کنیم؟
...
> ```
>
> ## <a id="python-cpython-update-schedule"></a> زمان‌بندی به‌روزرسانی CPython
>
