...
این آموزش فرض می‌کند که شما یک [کش باینری سازگار با S3](/pages/nix-manual/command-ref/new-cli/nix3-help-stores#s3-binary-cache-store) را به عنوان یک [substituter](/pages/nix-manual/command-ref/conf-file#conf-substituters) پیکربندی کرده‌اید و پروفایل پیش‌فرض AWS کاربر `root` می‌تواند به باکت (bucket) بارگذاری کند.
...
سپس [`nix.conf`](/pages/nix-manual/command-ref/conf-file) را روی هر ماشینی که قرار است به کش دسترسی داشته باشد، به‌روزرسانی کنید.
URL کش را به [`substituters`](/pages/nix-manual/command-ref/conf-file#conf-substituters) و کلید عمومی را به [`trusted-public-keys`](/pages/nix-manual/command-ref/conf-file#conf-trusted-public-keys) اضافه کنید:
...
روی آن ماشین‌ها، مسیر فایل کلید را به فیلد [`secret-key-files`](/pages/nix-manual/command-ref/conf-file#conf-secret-key-files) در [`nix.conf`](/pages/nix-manual/command-ref/conf-file) آن‌ها اضافه کنید:
