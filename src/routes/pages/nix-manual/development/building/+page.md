...
> اگر آن را نصب نکرده‌اید، دستورالعمل‌های [نصب](/pages/nix-manual/installation) را دنبال کنید.
...
برای کسب اطلاعات بیشتر درباره‌ی اجرا و فیلتر کردن تست‌ها، به [`testing.md`](/pages/nix-manual/development/testing) مراجعه کنید.
...
[remote builders]: /pages/nix-manual/language/derivations#attr-builder
...
- `stdenv`: پیش‌فرض؛
- `gccStdenv`: اجبار به استفاده از کامپایلر `gcc`؛
- `clangStdenv`: اجبار به استفاده از کامپایلر `clang`؛
- `ccacheStdenv`: فعال‌سازی [ccache]، یک کش کامپایلر برای سرعت بخشیدن به فرآیند ساخت.
