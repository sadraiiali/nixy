...
### <a id="string-literal"></a> رشته (String)

به [مقاله‌ی مقادیر ثابت رشته‌ای (String literals)](/pages/nix-manual/language/string-literals) مراجعه کنید.

### <a id="number-literal"></a> عدد (Number)
...
### <a id="path-literal"></a> مسیر (Path)
  ...
  *مسیرها* را می‌توان با استفاده از مقادیر ثابت مسیر مانند `./builder.sh` بیان کرد.
  ...
  مقادیر ثابت مسیر نسبت به [پوشه پایه](/pages/nix-manual/glossary#gloss-base-directory) خود حل می‌شوند.
  ...
  مقادیر ثابت مسیری که با `~` شروع می‌شوند، در ارزیابی [خالص (Pure)](/pages/nix-manual/command-ref/conf-file#conf-pure-eval) مجاز نیستند.
  ...
  عبارت `a.${'{'}'{'{'}'{'}'}foo{'{'}'{'}'}'{'}'}/b.$`یک عملیات تقسیم عددی از نظر نحوی معتبر است.
  در حالی که`./a.${'{'}'{'{'}'{'}'}foo{'{'}'{'}'}'{'}'}/b.$`یک مسیر است.

  مقادیر ثابت [مسیر جستجو](/pages/nix-manual/language/constructs/lookup-path) مانند`&lt;nixpkgs&gt;` نیز به مقادیر مسیر حل می‌شوند.

## <a id="list-literal"></a> فهرست (List)
...
به عناصر موجود در یک فهرست می‌توان با استفاده از [`builtins.elemAt`](/pages/nix-manual/language/builtins#builtins-elemAt) دسترسی پیدا کرد.

## <a id="attrs-literal"></a> مجموعه ویژگی (Attribute Set)
...
نام صفت یک رشته‌ی بدون کانتکست است و با یک [نام] (یک [شناسه](/pages/nix-manual/language/identifiers#identifiers) یا [رشته‌ی تحت‌الفظی](/pages/nix-manual/language/string-literals)) نشان داده می‌شود.

[name]: /pages/nix-manual/language/identifiers#names
> **نحو (Syntax)**
>
> *attrset* → `{'{'}'{'{'}'{'}'}` {'{'}'{'{'}'{'}'} *name* `=` *expr* `;` {'{'}'{'}'}'{'}'} `{'{'}'{'}'}'{'}'}`
...
به صفات می‌توان با استفاده از [عملگر `.`](/pages/nix-manual/language/operators#attribute-selection) دسترسی پیدا کرد.
...
مجموعه‌ای که دارای یک صفت [`__functor`]<a id="attr-__functor"></a> باشد که مقدار آن قابل‌فراخوانی است (یعنی خودش یک تابع یا مجموعه‌ای با یک صفت `__functor` است که مقدار آن قابل‌فراخوانی است)، می‌تواند طوری فراخوانی شود که انگار یک تابع است، در حالی که خود مجموعه به عنوان اولین آرگومان به آن ارسال می‌شود؛ برای مثال،
...
مجموعه‌های بازگشتی شبیه [مجموعه‌های ویژگی](/pages/nix-manual/language/types#type-attrs) معمولی هستند، با این تفاوت که ویژگی‌های درون آن‌ها می‌توانند به یکدیگر ارجاع دهند.
...
هنگام تعریف یک [مجموعه ویژگی](/pages/nix-manual/language/types#type-attrs) یا در یک [عبارت let](#let-expressions)، اغلب کپی کردن متغیرها از حوزه واژگانی پیرامون (مثلاً وقتی می‌خواهید صفات را منتشر کنید) راحت‌تر است.
...
- تمام اتصالات (bindings) معرفی‌شده توسط تابع، در سرتاسر عبارت تابع در محدودهٔ دید (scope) قرار دارند؛ و نه فقط در بدنهٔ آن.
