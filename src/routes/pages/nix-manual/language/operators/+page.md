...
[number]: /pages/nix-manual/language/types#type-float
...
- یک مجموعه ویژگی دارای [صفت `__functor`](/pages/nix-manual/language/syntax#attr-__functor)
...
همچنین ببینید: [`builtins.hasAttr`](/pages/nix-manual/language/builtins#builtins-hasAttr)
...
- تقسیم بر صفر
- سرریز عدد صحیح، یعنی هر عملیاتی که نتیجه‌ای خارج از محدوده قابل‌نمایش [اعداد صحیح زبان Nix](/pages/nix-manual/language/syntax#number-literal) تولید کند.
...
دو [رشته][string] را به هم متصل کرده و [زمینه‌های رشته‌ای](/pages/nix-manual/language/string-context) آن‌ها را ادغام کنید.
...
> **نکته**
>
> رشته نباید دارای [زمینه رشته‌ای](/pages/nix-manual/language/string-context) باشد که به یک [مسیر انبار] ارجاع دهد.
...
[store]: /pages/nix-manual/glossary#gloss-store
...
این عملگر در هر دو *attrset1* و *attrset2* [سخت‌گیرانه (strict)](/pages/nix-manual/language/evaluation#strictness) است.
این بدان معناست که هر دو آرگومان به [فرم نرمال سر ضعیف (weak head normal form)](/pages/nix-manual/language/evaluation#values) ارزیابی می‌شوند، بنابراین خود مجموعه‌های ویژگی ارزیابی می‌شوند، اما مقادیر صفات آن‌ها ارزیابی نمی‌شوند.
...
عملگر `==` در هر دو آرگومان [سخت‌گیرانه (strict)](/pages/nix-manual/language/evaluation#strictness) است؛ هنگام مقایسه انواع مرکب ([مجموعه‌های ویژگی][attribute set] و [فهرست‌ها][list])، در مقادیر درون آن‌ها به طور جزئی سخت‌گیرانه است: آن‌ها تا زمان یافتن اختلاف ارزیابی می‌شوند. 
...
[function]: /pages/nix-manual/language/syntax#functions
[Equality]: #equality
...
این عملگر در *bool1* [سخت‌گیرانه (strict)](/pages/nix-manual/language/evaluation#strictness) است، اما *bool2* را تنها در صورتی ارزیابی می‌کند که *bool1* برابر با `true` باشد.
...
این عملگر روی *bool1* [سخت‌گیرانه](/pages/nix-manual/language/evaluation#strictness) (strict) عمل می‌کند، اما *bool2* را تنها در صورتی ارزیابی می‌کند که مقدار *bool1* برابر با `false` باشد.
...
این عملگر نسبت به *bool1* [سخت‌گیرانه](/pages/nix-manual/language/evaluation#strictness) (strict) است، اما *bool2* را تنها در صورتی ارزیابی می‌کند که *bool1* برابر با `true` باشد.
...
> **هشدار**
>
> این سینتکس بخشی از یک [ویژگی آزمایشی](/pages/nix-manual/development/experimental-features) است و ممکن است در نسخه‌های آتی تغییر کند.
>
> برای استفاده از این سینتکس، مطمئن شوید که [ویژگی آزمایشی `pipe-operators`](/pages/nix-manual/development/experimental-features#xp-feature-pipe-operators) فعال است.
> برای مثال، مورد زیر را در [`nix.conf`](/pages/nix-manual/command-ref/conf-file) لحاظ کنید:
>
> ```
...
[توابع داخلی (builtins)]: /pages/nix-manual/language/builtins
