...
> Source: [/pages/nix-manual/language/builtins-prefix](/pages/nix-manual/language/builtins)
...
derivation is described in
 [its own section](/pages/nix-manual/language/derivations).
...
[constant](/pages/nix-manual/language/string-context#string-context-constant)
string context element is turned into a
[derivation deep](/pages/nix-manual/language/string-context#string-context-element-derivation-deep)
...
Return the *base name* of either a [path value](/pages/nix-manual/language/types#type-path) *x* or a string *x*, depending on which type is passed, and according to the following rules.
...
Since built-in functions were added over time, [testing for attributes](/pages/nix-manual/language/operators#has-attribute) in `builtins` can be used for graceful fallback on older Nix installations:
...
algorithm is the same as the one used by [`nix-env -u`](/pages/nix-manual/command-ref/nix-env/upgrade).
...
The value of the
[`eval-system`](/pages/nix-manual/command-ref/conf-file-prefix#conf-eval-system)
or else
...
**Note**
Not available in [pure evaluation mode](/pages/nix-manual/command-ref/conf-file-prefix#conf-pure-eval).
...
The [store path](/pages/nix-manual/store/store-path) of a derivation depending on `currentTime` differs for each evaluation, unless both evaluate `builtins.currentTime` in the same second.
...
[comparison operators](/pages/nix-manual/language/operators#comparison)
and used in
[conditional expressions](/pages/nix-manual/language/syntax#conditionals).
...
**Note**
This function is only available if the [`fetch-closure` experimental feature](/pages/nix-manual/development/experimental-features#xp-feature-fetch-closure) is enabled.
...
Fetch a store path [closure](/pages/nix-manual/glossary#gloss-closure) from a binary cache, and return the store path as a string with context.
...
use [`nix store make-content-addressed`](/pages/nix-manual/command-ref/experimental-commands):
...
Also, using content-addressed store paths does not require users to configure [`trusted-public-keys`](/pages/nix-manual/command-ref/conf-file-prefix#conf-trusted-public-keys) to ensure their authenticity.
...
**Note**
This function is only available if the [`fetch-tree` experimental feature](/pages/nix-manual/development/experimental-features#xp-feature-fetch-tree) is enabled.
For example, include the following in [`nix.conf`](/pages/nix-manual/command-ref/conf-file-prefix):
extra-experimental-features = fetch-tree

Fetch a file system tree or a plain file using one of the supported backends and return an attribute set with:

the resulting fixed-output [store path](/pages/nix-manual/store/store-path)
the corresponding [NAR](/pages/nix-manual/store/file-system-object/content-address#serial-nix-archive) hash
...
If `narHash` is set, the source is first looked up is the Nix store and [substituters](/pages/nix-manual/command-ref/conf-file-prefix#conf-substituters), and only fetched if not available.
...
A NAR hash is supplied and the corresponding store path is not [valid](/pages/nix-manual/glossary#gloss-validity), that is, not available in the store

**Note**
[Substituters](/pages/nix-manual/command-ref/conf-file-prefix#conf-substituters) are not used in fetching.

There is no cache entry or the cache entry is older than [`tarball-ttl`](/pages/nix-manual/command-ref/conf-file-prefix#conf-tarball-ttl)
...
This is similar to [`builtins.fetchurl`](/pages/nix-manual/language/builtins-prefix#builtins-fetchurl)
...
This is similar to [`builtins.fetchGit`](/pages/nix-manual/language/builtins-prefix#builtins-fetchGit).
...
This has the same underlying implementation as [`builtins.fetchTarball`](/pages/nix-manual/language/builtins-prefix#builtins-fetchTarball)
...
*input* can also be a [URL-like reference](/pages/nix-manual/command-ref/new-cli/nix3-flake#flake-references).
The additional input types and the URL-like syntax requires the [`flakes` experimental feature](/pages/nix-manual/development/experimental-features#xp-feature-flakes) to be enabled.
...
[Lookup path](/pages/nix-manual/language/constructs/lookup-path) expressions are [desugared](https://en.wikipedia.org/wiki/Syntactic_sugar) using this and `builtins.nixPath`:
&lt;nixpkgs&gt;

is equivalent to:
builtins.findFile builtins.nixPath "nixpkgs"

A search path is represented as a list of [attribute sets](/pages/nix-manual/language/types#type-attrs) with two attributes:
...
See [documentation on `nix-channel`](/pages/nix-manual/command-ref/nix-channel) for details about channels.
...
**Note**
This function is only available if the [`flakes` experimental feature](/pages/nix-manual/development/experimental-features#xp-feature-flakes) is enabled.
For example, include the following in [`nix.conf`](/pages/nix-manual/command-ref/conf-file-prefix):
...
[Int](/pages/nix-manual/language/types#type-int)
[Float](/pages/nix-manual/language/types#type-float)
[Boolean](/pages/nix-manual/language/types#type-bool)
[String](/pages/nix-manual/language/types#type-string)
[Path](/pages/nix-manual/language/types#type-path)
[List](/pages/nix-manual/language/types#type-list)
...
The *path* argument must meet the same criteria as an [interpolated expression](/pages/nix-manual/language/string-interpolation#interpolated-expression).
...
A list of search path entries used to resolve [lookup paths](/pages/nix-manual/language/constructs/lookup-path).
Its value is primarily determined by the [`nix-path` configuration setting](/pages/nix-manual/command-ref/conf-file-prefix#conf-nix-path), which are

Overridden by the [`NIX_PATH`](/pages/nix-manual/command-ref/env-common#env-NIX_PATH) environment variable or the `--nix-path` option
Extended by the [`-I` option](/pages/nix-manual/command-ref/opt-common#opt-I) or `--extra-nix-path`
...
**Note**
This function is only available if the [`dynamic-derivations` experimental feature](/pages/nix-manual/development/experimental-features#xp-feature-dynamic-derivations) is enabled.
For example, include the following in [`nix.conf`](/pages/nix-manual/command-ref/conf-file-prefix):
extra-experimental-features = dynamic-derivations

Return the output path of a derivation, literally or using an
[input placeholder string](/pages/nix-manual/store/derivation#input-placeholder)
if needed.
...
This primop corresponds to the `^` sigil for [deriving paths](/pages/nix-manual/glossary#gloss-deriving-path), e.g. as part of installable syntax on the command line.
...
[flat hash](/pages/nix-manual/store/file-system-object/content-address#serial-flat),
rather than a hash of the
[NAR serialization](/pages/nix-manual/store/file-system-object/content-address#serial-nix-archive)
...
[content hash](/pages/nix-manual/store/file-system-object/content-address)
...
Return an
[output placeholder string](/pages/nix-manual/store/derivation#output-placeholder)
for the specified *output* that will be substituted by the corresponding
[output path](/pages/nix-manual/glossary#gloss-output-path)
at build time.
...
[`nix-env -u`](/pages/nix-manual/command-ref/nix-env/upgrade).

`storeDir` (string)

Logical file system location of the [Nix store](/pages/nix-manual/glossary#gloss-store) currently in use.
This value is determined by the `store` parameter in [Store URLs](/pages/nix-manual/store/types#store-url-format):
...
Note that `$`is a
[string interpolation](/pages/nix-manual/language/types#type-string), so the result of the
...
stylesheet is spliced into the builder using the syntax `xsltproc $`.`trace e1 e2`

Evaluate *e1* and print its abstract syntax representation on
standard error. Then return *e2*. This function is useful for
debugging.
If the
[`debugger-on-trace`](/pages/nix-manual/command-ref/conf-file-prefix#conf-debugger-on-trace)
option is set to `true` and the `--debugger` flag is given, the
interactive debugger is started when `trace` is called (like
[`break`](/pages/nix-manual/language/builtins-prefix#builtins-break)).
...
Discard the [string context](/pages/nix-manual/language/string-context) from a value that can be coerced to a string.
...
If the
[`debugger-on-trace`](/pages/nix-manual/command-ref/conf-file-prefix#conf-debugger-on-trace)
or [`debugger-on-warn`](/pages/nix-manual/command-ref/conf-file-prefix#conf-debugger-on-warn)
option is set to `true` and the `--debugger` flag is given, the
interactive debugger will be started when `warn` is called (like
[`break`](/pages/nix-manual/language/builtins-prefix#builtins-break)).
If the
[`abort-on-warn`](/pages/nix-manual/command-ref/conf-file-prefix#conf-abort-on-warn)
