# 4.4.1.2. خروجی‌های derivation آدرس‌دهی‌شده بر اساس ورودی
...
## <a id="hash-quotient-drv"></a> خروجی‌های derivation با آدرس‌دهی محتوایی پیمانه‌ای (Modulo)
...
بنابراین، [بخش هش](/pages/nix-manual/store/store-path#digest) مسیرهای خروجی یک derivation با آدرس‌دهی ورودی را چگونه محاسبه می‌کنیم؟
این کار توسط تابع `hashQuotientDerivation` که در ادامه نشان داده شده است، انجام می‌شود.

ابتدا، نکته‌ای درباره ورودی‌ها.
تابع `hashQuotientDerivation` تنها روی derivationهایی تعریف می‌شود که [ورودی‌هایشان](/pages/nix-manual/store/derivation#inputs) شکل مرتبه اول (first-order) داشته باشند:
...
آن دسته از derivationها باید ابتدا به اندازه کافی (تا حدودی) [رزولوشن یا حل شوند](/pages/nix-manual/store/resolution)، تا حدی که هیچ‌گونه ورودی مرتبه بالای این‌چنینی باقی نماند.
...
درایویشن شبه‌مانند حاصل (که به جای مسیرهای انبار در `inputDrvs` دارای هش‌ها است) سپس چاپ می‌شود (در [فرمت "ATerm"](/pages/nix-manual/protocols/derivation-aterm)) و هش می‌شود، و این به عنوان هش «درایویشن خارج‌قسمتی» (quotient derivation) در نظر گرفته می‌شود.
...
اگر خروجی‌ها [آدرس‌دهی‌شده بر اساس محتوا](/pages/nix-manual/store/derivation/outputs/content-address) باشند، آنگاه برای هر خروجی، هشی مشتق‌شده از آدرس محتوای آن خروجی محاسبه می‌شود.

> **نکته**
>
> در حالت آدرس‌دهی‌شده بر اساس محتوای [ثابت](/pages/nix-manual/store/derivation/outputs/content-address#fixed)، آدرس‌های محتوای خروجی‌ها از پیش به صورت ایستا مشخص می‌شوند، بنابراین این کار همیشه به درستی انجام می‌شود.
> (حالت ثابت همان چیزی است که شبه‌کد نشان می‌دهد.)
>
> در حالت [شناور](/pages/nix-manual/store/derivation/outputs/content-address#floating)، آدرس‌های محتوا از پیش مشخص نمی‌شوند.
> این همان چیزی است که نظر «یا در صورت شناور بودن، دریافت از [ردیابی ساخت](/pages/nix-manual/store/build-trace)» به آن اشاره دارد.
...
ممکن است به ازای هر مسیری که از طریق آن می‌توان به یک زیردرایویشن (subderivation) رسید، خودش را فراخوانی کند؛ یعنی برای یک گراف درایویشن با `V` درایویشن و درجهٔ خروجی حداکثر `k`، به میزان `O(V^k)` بار فراخوانی شود.
...
*پیوست این فصل را دربارهٔ قراردادهای گرامر و متاوریاست‌ها (metavariables) در [store/math-notation.md ببینید](/pages/nix-manual/store/math-notation).*
...
ما می‌توانیم این رابطهٔ هم‌ارزی را مستقیماً و با کار کردن از پایین به بالا مشخص کنیم.

ما با تعریف یک رابطهٔ هم‌ارزی روی مسیرهای اشتقاق خروجی مرتبه اول شروع می‌کنیم که به خروجی‌های درایویشن محتوا-محور (content-addressed) ارجاع می‌دهند. دو مسیر از این دست معادل هستند اگر به یک شیء انبار یکسان ارجاع دهند:
...
که در آن \\({'{'}{'}'}^*(s, o)\\) نشان‌دهندهٔ شیء انباری است که مسیر اشتقاق خروجی به آن ارجاع می‌دهد.

ما همچنین به ساختار زیر نیاز خواهیم داشت تا هر رابطهٔ هم‌ارزی روی \\(X\\) را به یک رابطهٔ هم‌ارزی روی مجموعه‌های (متناهی) از \\(X\\) (به‌طور خلاصه، \\(\\mathcal{'{'}P{'}'}(X)\\)) ارتقا دهیم:
...
>
> \\(\\sim_\mathrm{'{'}Drv{'}'}\\) از [رزولوشن درایویشن](/pages/nix-manual/store/resolution) چنین رابطه هم‌ارزشی است.
...
[xp-feature-ca-derivations]: /pages/nix-manual/development/experimental-features#xp-feature-ca-derivations
