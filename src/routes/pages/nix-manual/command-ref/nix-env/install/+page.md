...
- به‌طور پیش‌فرض، *args* مجموعه‌ای از نام‌ها است که نمایانگر derivationها در [default Nix expression] هستند.
  ...
  [derivation expression]: glossary.md#gloss-derivation-expression
...
- اگر [`--attr`](#opt-attr) / `-A` مشخص شده باشد، آرگومان‌ها *مسیرهای صفت (attribute paths)* هستند که صفت‌ها را از [default Nix expression] انتخاب می‌کنند.
  ...
  مسیرهای صفت بسته‌های موجود را با [`nix-env --query`](/pages/nix-manual/command-ref/nix-env/query) نمایش دهید:
...
- اگر `--from-expression` داده شود، *args* [Nix language functions](/pages/nix-manual/language/syntax#functions) هستند که با [default Nix expression] به عنوان تنها آرگومانشان فراخوانی می‌شوند.
...
- به طور پیش‌فرض، تمام [outputs](/pages/nix-manual/language/derivations#attr-outputs) برای هر [store derivation] نصب می‌شوند.
