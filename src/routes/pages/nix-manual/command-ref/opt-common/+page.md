...
بنابراین اگر این عبارت Nix را فراخوانی کنید (مثلاً هنگامی که `nix-env --install --attr pkgname` را اجرا می‌کنید)، تابع به‌طور خودکار با استفاده از مقدار [`builtins.currentSystem`](/pages/nix-manual/language/builtins) برای آرگومان `system` فراخوانی خواهد شد.
...
- <span id="opt-attr">[`--attr`](#opt-attr)</span> / `-A` *attrPath*
  ...
  برای چند نمونه عینی به [`nix-env --install`](/pages/nix-manual/command-ref/nix-env/install) مراجعه کنید.
...
> **نکته**
>
> برای بازنشانی تنظیمات پیکربندی با استفاده از پرچم‌های خط فرمان، به [`man nix.conf`](/pages/nix-manual/command-ref/conf-file#command-line-flags) مراجعه کنید.
