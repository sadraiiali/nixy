...
فایل مانیفست، منشأ (provenance) بسته‌هایی را که در یک [پروفایل](/pages/nix-manual/command-ref/files/profiles) مدیریت‌شده توسط [`nix profile`](/pages/nix-manual/command-ref/new-cli/nix3-profile) (تجربی) نصب شده‌اند، ثبت می‌کند.
...
* `originalUrl`: [مرجع فلیک](/pages/nix-manual/command-ref/new-cli/nix3-flake) مشخص‌شده توسط کاربر در زمان نصب (به‌عنوان‌مثال `nixpkgs`). این همان مرجع فلیک است که توسط `nix profile upgrade` استفاده خواهد شد.
