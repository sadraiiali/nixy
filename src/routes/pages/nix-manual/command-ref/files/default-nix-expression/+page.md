# 8.6.4. عبارت پیش‌فرض Nix

## عبارت Nix پیش‌فرض

سورسِ [عبارت‌های نیکس (Nix expression)](/pages/nix-manual/glossary#gloss-nix-expression) که به طور پیش‌فرض توسط [`nix-env`] استفاده می‌شود:
...
فایل [`manifest.nix`](/pages/nix-manual/command-ref/files/manifest.nix) همیشه نادیده گرفته می‌شود.
...
[channels]: /pages/nix-manual/command-ref/files/channels
