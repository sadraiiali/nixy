...
> **نکته**
>
> در حالی که انبار Nix مورد استفاده روی ماشین محلی را می‌توان در خط فرمان با گزینه [`--store`](/pages/nix-manual/command-ref/conf-file#conf-store) مشخص کرد، انبار Nix قابل دسترس روی ماشین راه دور تنها می‌تواند روی همان ماشین راه دور [پیکربندی استاتیک](/pages/nix-manual/command-ref/conf-file#configuration-file) شود.
...
- `--from`
  ...
  کپی کردن closure مربوط به _paths_ از انبار Nix روی _machine_ راه دور به انبار Nix مشخص‌شده‌ی ماشین محلی.
...
- `--use-substitutes` / `-s`
  ...
  تلاش برای بارگیری شیءهای انبار مفقود روی مقصد از [substituters](/pages/nix-manual/command-ref/conf-file#conf-substituters).
