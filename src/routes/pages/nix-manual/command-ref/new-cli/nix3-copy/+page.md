...
> Source: [/pages/nix-manual/command-ref/new-cli/nix3-copy](/pages/nix-manual/command-ref/new-cli/nix3-copy)
...
- `--include` / `-I` *path*
Add *path* to search path entries used to resolve [lookup paths](/pages/nix-manual/language/constructs/lookup-path)
This option may be given multiple times.
Paths added through `-I` take precedence over the [`nix-path` configuration setting](/pages/nix-manual/command-ref/conf-file-prefix#conf-nix-path) and the [`NIX_PATH` environment variable](/pages/nix-manual/command-ref/env-common#env-NIX_PATH).
...
**DEPRECATED**
Use [`--no-use-registries`](/pages/nix-manual/command-ref/conf-file-prefix#conf-use-registries) instead.
- `--no-update-lock-file`
...
**DEPRECATED**
Use [`nix flake update`](/pages/nix-manual/command-ref/experimental-commands) instead.
...
## <a id="options-that-change-the-interpretation-of-installables"></a> Options that change the interpretation of [installables](/pages/nix-manual/command-ref/experimental-commands#installables)

- `--all`
Apply the operation to every store path.
- `--derivation`
Operate on the [store derivation](/pages/nix-manual/glossary#gloss-store-derivation) rather than its outputs.
- `--expr` *expr*
Interpret [*installables*](/pages/nix-manual/command-ref/experimental-commands#installables) as attribute paths relative to the Nix expression *expr*.
- `--file` / `-f` *file*
Interpret [*installables*](/pages/nix-manual/command-ref/experimental-commands#installables) as attribute paths relative to the Nix expression stored in *file*. If *file* is the character -, then a Nix expression is read from standard input. Implies `--impure`.
- `--no-recursive`
