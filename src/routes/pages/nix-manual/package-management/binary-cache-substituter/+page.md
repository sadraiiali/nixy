# 7.1.1. میزبانی انبار Nix از طریق HTTP
...
برای بررسی اینکه آیا به درستی کار می‌کند، تلاش کنید فایل [`nix-cache-info`](/pages/nix-manual/protocols/binary-cache/nix-cache-info) را روی کلاینت دریافت کنید:
...
هنگام نوشتن در یک کش باینری (مثلاً با [`nix copy`](/pages/nix-manual/command-ref/new-cli/nix3-copy))، اگر فایل [`nix-cache-info`](/pages/nix-manual/protocols/binary-cache/nix-cache-info) وجود نداشته باشد، Nix آن را به‌طور خودکار ایجاد می‌کند.
