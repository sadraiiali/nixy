...
حذف یک [نصب چندکاربره](/pages/nix-manual/installation/installing-binary#multi-user-installation) به سیستم‌عامل بستگی دارد.
...
۴. همچنین ممکن است ارجاعاتی به Nix در موارد زیر وجود داشته باشد:
   ...
   - `/usr/local/etc/zshrc`
   - فایل‌های پیکربندی شل در پوشه‌های خانهٔ کاربران
...
> ```
>
> هنگام اجرای دستورات Nix، برای اطلاع از دستورالعمل‌های رفع مشکل نصب بدون نیاز به نصب مجدد، به مسألهٔ گیت‌هاب [NixOS/nix#10892](https://github.com/NixOS/nix/issues/10892) مراجعه کنید.
...
> این نشانهٔ انتظاری از یک حذف نصب موفق است.
...
> سیستم‌عامل macOS (نسخه Catalina به بعد) مستقیماً پوشه‌های ریشه را کنترل می‌کند و ریشهٔ فقط‌خواندنی آن مانع از حذف دستی نقطه اتصال `/nix` خالی می‌شود.

## تک‌کاربره

برای حذف یک [نصب تک‌کاربره](/pages/nix-manual/installation/installing-binary#single-user-installation) از Nix، دستور زیر را اجرا کنید:
